...
- `View`: delete `rotationX`/`rotationY`/`rotationZ` lines 145-170
- `EffectManager`:
	- delete `rotationX`/`rotationY`/`rotationZ` lines 87-89
	- delete `scaleZ` lines 51;85
- `MovieClipCutsceneRenderer`: delete soundTransform lines 155;160
- `MainPanel`: delete "Security.showSettings(...)" line ~1170 (it doesn't do anything in AIR, it's only for Flash)
- `CommandLine`: delete `setPropertyIsEnumerable` line ~210 (not available in OpenFL, possible to implement but not urgent)

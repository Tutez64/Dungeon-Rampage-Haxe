...
**v1 is facade + host types.** The facade is the recommended path. `extends` / `replace` exist so a mod can still reach what wrappers do not cover. That power is a v1 foundation. Both land in the **hxScript fork**. The fork is pinned by commit in `project.xml`, rebased on upstream, PR'd once it works for DRH. hxScript's compiler is BETA; `replace` touches bridge generation and the emitter, so rebases will conflict more than on lime/openfl. Accepted: the goal is a clean `replace`, not the smallest diff.

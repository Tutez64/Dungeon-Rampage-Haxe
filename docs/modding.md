...
| Mod `id` and zip extract | decided | `id` is `^[a-z][a-z0-9_]{1,62}[a-z0-9]$` (3–64, snake_case, starts with a letter, no trailing underscore) **and not a Haxe keyword or a Windows reserved device name** — a valid Haxe package segment and a creatable folder, so `id` = install folder = the mod's package `mods.<id>` with no conversion anywhere. Same list in index CI, launcher, host. Zip install reuses the game-archive extractor (no zip-slip). No uncompressed size cap. See [`mod.json`](#modjson). |
| `dependencies` cycles | decided | A cycle is **not** an error: in one shared batch mutually importing mods compile. It only leaves the `onInit` order undefined. The launcher collapses the cycle into one block, keeps the user's order inside it, and warns. Never refuses to enable. See [Passing the list](#passing-the-list). |
| `import.hx` in a mod | decided | **Skipped** by the host with a warning attributed to the mod. hxScript's prelude (`ImportModule`) is root-level, host-wired, and interpreter-only — the cppia emitter reads a module's own `import`s only — so honouring it would leave every module of that mod interpreted. Each file writes its imports. See [`mod.json`](#modjson). |
...
| `onDispose()` | On `NativeApplication` **`exiting`**, registered at the **top** of the `DungeonBustersProject` constructor (before compile, ahead of the game's own listener, so **`onDispose` runs before `mSteamworks.dispose()`**). Called once, in reverse load order. Every graceful exit reaches it (window close, `WM_CLOSE`, `SIGTERM`, Cmd+Q, Quit, socket error). The launcher's Stop force-kills after **3 s**, so a mod gets a synchronous, local cleanup and no more. Not called on a crash or a force-kill, nor if the loop never pumps the quit (compile stuck in the constructor, hung frame). | Timers, listeners, flush a local file. No network, no waiting on anything. Must not block exiting. |
...
<install-dir>/
  ...
  mods/
    ...
    some_mod/         # folder name = id
      mod.json
      src/            # .hx sources; package mods.some_mod (+ subfolders); no import.hx (skipped, warned)
...
**Cycles.** The sort is on strongly connected components, not on a graph assumed acyclic: a cycle becomes one block, ordered as a unit against the rest, with the user's order kept **inside** it, and the Mods page shows a warning naming the members (`a ↔ b: load order between them is yours`). Enabling is never refused for it. That matches what actually happens in the game: with one shared `Environment` and one batch, `a` importing `b` and `b` importing `a` both compile ("every module is declared before any is emitted"); the only thing a cycle leaves undefined is which `onInit` runs first, and the user's order is a reasonable answer to that. The host never sorts; it follows `enabled.json` as written.
...
| Field | Role |
| --- | --- |
| `id` | Stable identifier, install folder name, and the mod's **package**: every source file is in `mods.<id>` or a sub-package of it. Must match `^[a-z][a-z0-9_]{1,62}[a-z0-9]$` (3–64 chars, snake_case, starts with a letter, no trailing underscore) and must **not** be a Haxe keyword (`abstract`, `break`, `case`, `cast`, `catch`, `class`, `continue`, `default`, `dynamic`, `else`, `enum`, `extends`, `extern`, `false`, `final`, `for`, `function`, `implements`, `import`, `inline`, `interface`, `macro`, `new`, `null`, `operator`, `overload`, `override`, `package`, `private`, `public`, `return`, `static`, `switch`, `this`, `throw`, `true`, `try`, `typedef`, `untyped`, `using`, `var`, `while` — the regex alone lets `package mods.new;` through) nor a Windows reserved device name (`con`, `prn`, `aux`, `nul`, `com1`–`com9`, `lpt1`–`lpt9` — the folder cannot be created). With those two lists the `id` is exactly a valid Haxe package segment and a creatable folder, so nothing is converted anywhere. Index CI, launcher install, and host skip all apply the same regex + lists. Display name stays in `name`. |
| `name` | Display name |
| `version` | Mod version |
| `author` | Author |
...
| `dependencies` | Ids of mods whose script types this one imports (`import mods.cool_lib.Api;`). **Ids only, no versions.** Used for two things: the launcher orders `enabled.json` so dependencies load first, and warns when one is not enabled. Nothing else: no auto-install, no version solving. May be empty or absent. A **cycle** (`a` depends on `b`, `b` on `a`) is not an error and does not block enabling: inside one batch both still compile; only the `onInit` order between them is undefined, so the launcher keeps the user's order for the cycle's members and shows a warning. |
...
**`import.hx` is not supported in a mod (v1).** The host's walk of `src/**/*.hx` skips any `import.hx`, the same way hxScript's own `Bridges` and `Autowire` scans do, and logs one warning attributed to the mod (`import.hx ignored`). Not a `failed`: the other files still load. The reason is hxScript's, not ours: its prelude mechanism is `hxscript.runtime.ImportModule`, a single root-level module (`path = "import"`, empty package) that the **embedder** pushes into each `Module.subModules`; `Module.start` folds its imports/usings into the interpreter. The cppia `Emitter.declare` reads a module's **own** `DImport` declarations only, so a module relying on a prelude would fail name resolution at compile and run interpreted — every file of that mod, silently slower. Fed as a normal module instead, the file would fail on its own: Haxe's `import.hx` carries no `package` line, so it declares the root package where the host expects `mods.<id>` — the same parse error as any misplaced file — and the whole mod would be `failed` for a file that never was a type. Skipping is the honest outcome. Each file lists its imports. If hxScript later teaches the emitter about preludes, lifting this is additive.
...
- Enable / disable, load order (topological on `dependencies`, the user's order where free; a cycle is one block in the user's order plus a warning, never a refusal; warn on a dependency that is not enabled). Refuse to install an `id` that fails the regex, is a Haxe keyword, or a Windows reserved name. `drh` is required on every mod. Warn if the installed tag is older than the set (all kinds) or newer (`extends` / `replace` only). Never refuse to enable or Play for that.
...
- Parse `--mods-dir` from `Sys.args()` in the constructor (like `--fps`). Without the flag, load nothing. Read `enabled.json`. Skip an entry whose `id` is not the snake_case regex (or is a Haxe keyword / Windows reserved name), whose folder / `mod.json` is missing, or whose `id` was already loaded (log + `last-run` `skipped`).
- Create **one** hxScript `Environment`. For each enabled mod, in `enabled.json` order, add its `src/**` modules with package `mods.<id>` + sub-folders, skipping any `import.hx` with a warning; a file declaring another package is hxScript's parse error and that mod is `failed`. Then **one** `Compiler.compile(env)` before `onInit`; interpret what was skipped. Attribute each module's compiled / skipped status to its mod by package prefix.
- Call [lifecycle](#lifecycle): `onInit` in the constructor before `new DBFacade()`, `onReady` in `architectureLoaded` before `run()` and `mainStateMachine.start()`, `tablesLoaded` on `ManagersLoadedEvent`, gameplay events when heroes/floors appear, `onDispose` on `NativeApplication` `exiting` (registered at the top of the constructor, so before the game's own listener and `mSteamworks.dispose()`), reverse load order. Keep the overlay root above the facade's layers (`addRootDisplayObject`, above the letterbox).

...
- **F — host types (hxScript fork).** Two features, both required for v1:
  1. **Bridge every game and engine class the repository ships.** The rule: a scriptable base is any eligible class in code that ends up in the binary **as game or engine code** — DRH's own source roots and the vendored engine submodules. Not bridged: the **toolchain** (hxcpp, hxScript, the Haxe std) and haxelib dependencies pulled in transitively (`format`). Nobody can rule out a use for a package inside that boundary; a curated list puts the burden of proof on the wrong side. Concretely:
     ...
     | `compat/` | `compat.*` plus 15 **root-level** classes (`ASCompat`, `ASDictionary`, `ASProxyBase`, …) | DRH code; `ASAny` / `ASObject` / `ASFunction` are abstracts and drop out of `eligible()` on their own |
     ...
     | haxelib `format` | `format` | transitive dependency, not vendored, not modified by us. Add it the day someone has a use |

     "Not bridged" means **no `extends`**, not "unreachable": a mod calls any host static directly, bridge or not. `cpp.vm.Gc.run`, `setMinimumWorkingMemory`, `enterGCFreeZone`, `memInfo`, … are all available to a mod that tunes the collector — the GC *algorithm* is C++ and compile-time defines (`HXCPP_GC_GENERATIONAL`, …), out of reach of any mod by nature. What protects those statics is not bridging but the build's **DCE and include** settings (see [Compilation](#compilation)): `-dce no` keeps every member of every typed std class, and `-D hxscript_keep=cpp.vm.Gc,cpp.vm.Profiler` forces those two to be typed in the first place, since `cpp.*` is not force-included wholesale.

     Mechanics: the submodules each have one root package, so stock `-D hxscript_bridge_packages=openfl,lime,swf,steamwrap` covers them (`Bridges.scanned`: named packages are taken as written, not matched against a library). DRH's own roots need what stock lacks, a **classpath-entry scan** — the empty root would walk the whole classpath, std included — so the fork adds `-D hxscript_bridge_classpath=src,src-steam,compat` (walk those source roots, bridge every eligible module) and `-D hxscript_bridge_exclude=<packages or types>`. The 15 root-level classes of `compat/` are the concrete case no package scan can reach. This is convenience for `src/` (the 35-package list works on stock hxScript today) but necessary for `compat/`; either way the scan is there so a new package is never forgotten. Implementation notes for the fork: the define names **classpath entries** to walk with an **empty package**, not a package called `src` — reusing `modulesUnder("src")` would look for `src/src/` and emit `src.actor.Hero`; and the walk skips `*.macro.hx` (`ASCompat.macro` resolves to nothing and only pollutes `-D hxscript_verbose`). `-D hxscript_host` stays what it is upstream: the packages scanned for `@:scriptAmbient` / `@:scriptStatic` source metadata, i.e. `modding`.

     One exclusion by default: **`DungeonBustersProject`**. Lime's generated `ApplicationMain` constructs it **before** the host exists (the host runs inside its constructor), so `replace` is impossible by construction, and a mod `extends` would be a second application instance. Its bridge would be the whole `Sprite` surface through `GameEntry` for a use that cannot exist. The other two root classes of `src/` (`Loading_screen_swf`, `Db_UI_skip_button_swf`, `@:bind` `MovieClip`s; `import.hx` is not a type) stay in: they are instantiated through `ASCompat.createInstance`, which the host routes through the `replace` table (see below), so they are as reachable as any `MovieClip` in `uI/`.

     Stock hxScript only lists `Sprite` as an OpenFL base and **no** Lime bases: that is a **default-size curation**, not a hard limit. `DisplayObject` does bridge (awkward methods fall through to `super`); the cost is one generated override per inherited method on a very wide surface. (`-D scriptable` is hxcpp's cppia flag; it does not generate extend bridges.) Know what a package scan means: `Bridges.scanned` walks the **whole** tree of a named package, with none of the presets' ignore lists, so `openfl._internal`, `lime._internal` and the backends are bridged too. The reason to keep that breadth is the boundary rule above: an experimental or too-specific optimisation may be easier to ship as a mod than as a fork rebuild, even if cppia in a hot path often loses to a native build. **Measure first, do not pre-empt:** the first cppia build records binary size and build time against current DRH. If it is too fat, the lever is `hxscript_bridge_exclude` on packages with no modding value (`generatedCode` network classes, `box2D`, `openfl._internal`, Lime backends), not `final` on DRH types one by one. Narrowing `openfl,lime` to display roots is a fallback after that measurement, not the v1 default.
  2. **`replace`.** Explicit `replace(HostClass, Sub)` so host `new HostClass(...)` constructs the subclass. **`extend` is not `replace`.** `class HudIcon extends Sprite` is a widget; it does not steal OpenFL's `new Sprite()`. Only an explicit `replace(Sprite, …)` would. Same rule for every type, including OpenFL/Lime: no special blacklist. Disjoint-method merge lives in hxScript; DRH just calls `replace` and gets one class or a conflict. A `new` rewrite does not see dynamic instantiation. DRH's own goes through **`ASCompat.createInstance`** (`compat/ASCompat.hx`, our code): `AssetRepository`, the loading clip, the skip button, every `var X:Dynamic = SomeClass` pattern. The host makes `createInstance` consult the same `replace` table, so the rule "`replace` covers every host construction" holds for DRH-side code. What remains outside is Lime's own `Type.createInstance` (e.g. `AnimateLibrary` from the asset manifest): `extends` works there, `replace` does not.
...
| --- | --- | --- |
| `onInit(ctx:ModContext)` | In the `DungeonBustersProject` constructor, after `--mods-dir` parse and compile, **before `new DBFacade()`**. The process and `stage` are up; **nothing of the game is constructed yet** (no facade, no `Logger`, no clocks, no camera, no state machine, no Steam; the `FeatureFlags` object does not exist and its values come later anyway). | `replace(...)`, subscribe to events, overlay on `stage`. No GameMaster yet; state wrappers are empty. Read feature flags in `onReady`, not here. |
...
Why that early, and why not a second hook. Be precise about what the placement buys: the HUD, sound, Steam Input, menus and `MainStateMachine` are all constructed **after** the constructor returns (`stagetwo_init`, `buildEngines`, `createHUD`), so an `onInit` at the end of `onInvoke` would already have covered them. What only "before `new DBFacade()`" covers is what `Facade.init` / `DBFacade.init` build synchronously: `DBFacade` itself, both `GameClock`s, `EventManager`, `Camera` (with `BackgroundFader`, `LetterboxEffect`), the work managers, the letterbox, the loading clip and its `UIProgressBar`. `Camera` and `GameClock` are plausible `replace` targets (zoom, shake, tick rate); the rest is marginal. The real argument is the rule: **"`replace` covers every host `new`"** is one sentence; "every host `new` except the eight classes `init()` constructs" is a list that moves with every boot refactor (Starling). A dedicated early hook would add a second "what exists here" rule on top. Two stable anchors instead: "before anything" (`onInit`) and "everything singleton is up, the loop has not ticked" (`onReady`). What `onInit` gives up by moving is weak: Steam identity is asynchronous anyway, and feature flags are only complete after `LoadingState.configReady` (config over CLI), so `onReady` was already the honest place to read them. `AssetRepository`, the loading clip and the skip button are built through `ASCompat.createInstance`, which the host routes through the `replace` table, so they are covered too (see [API surface](#api-surface)).

Why `onReady` is `LoadingFinishedEvent` and not `ManagersLoadedEvent`: the facade promises an **inventory** wrapper, and inventory is account data that only exists once `LoadingFinishedEvent` fires. At `ManagersLoaded` the tables are in but the account, matchmaker, clock and network are not — the same half-ready ambiguity we removed from `onInit`. The only thing that window is good for is mutating GameMaster before the account is parsed against it (an `extends` use); that is the additive `tablesLoaded` event below, not a frozen method. No fourth method (`onStart` next to a not-ready `onReady` is what this avoids).
...
-dce no                                              # hxScript: "not tuning" for cppia (see DCE below)
--macro include('haxe', true, ['haxe.macro'])         # force-type the std; ignore list grows with what fails on hxcpp

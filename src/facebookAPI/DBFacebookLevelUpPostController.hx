...
class DBFacebookLevelUpPostController {
	...
	public function new(dbFacade:DBFacade, feedPostCallback:ASFunction) {
		mDBFacade = dbFacade;
		mAssetLoadingComponent = new AssetLoadingComponent(mDBFacade);
		mEventComponent = new EventComponent(mDBFacade);
		mSceneGraphComponent = new SceneGraphComponent(mDBFacade, "DBFacebookLevelUpPostController");
		mFeedPostCallbackFunction = feedPostCallback;
	...
	public static function makeCurtain(dbFacade:DBFacade, sceneGraphComponent:SceneGraphComponent):Sprite {
		if (dbFacade == null || sceneGraphComponent == null) {
			return null;
		}
		...
		_loc3_.graphics.drawRect(0, 0, dbFacade.viewWidth, dbFacade.viewHeight);
		_loc3_.graphics.endFill();
		sceneGraphComponent.addChild(_loc3_, (105 : UInt));
		return _loc3_;
	}

	public static function removeCurtain(curtain:Sprite, sceneGraphComponent:SceneGraphComponent) {
		if (curtain == null || sceneGraphComponent == null) {
			return;
		}
		if (sceneGraphComponent.contains(curtain, (105 : UInt))) {
			sceneGraphComponent.removeChild(curtain);
		}
		curtain = null;
	}
	...
	function showPopup(levelUpPost:GMFeedPosts) {
	...
	function checkToSharePopup(event:Event) {
		checkIfMapIsLoaded();
	...
	function setlevelUpPost(event:FacebookLevelUpPostEvent) {
		checkIfMapIsLoaded();
		if (mLevelUpFeedPostsMap.hasKey(event.level)) {
			mSelectedLevelUpPost = ASCompat.dynamicAs(mLevelUpFeedPostsMap.itemFor(event.level), gameMasterDictionary.GMFeedPosts);
		}
	}

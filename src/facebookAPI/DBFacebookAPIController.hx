...
class DBFacebookAPIController extends FacebookAPIController {
	...
	static inline final dbFacebook_script_js:compat.XML = /*<script>
			<![CDATA[
			function()
			{
				DBFacebook =
	...
	public function new(dbFacade:DBFacade) {
		var _loc2_ = 0;
		super(dbFacade, dbFacade.facebookId);
	...
	public static function earnCredits(dbFacade:DBFacade, callback:ASFunction) {
		...
		Logger.debug("earnCredits: " + Std.string(dbFacade.dbAccountInfo.id));
		var _loc3_ = dbFacade.facebookController.appUrl + "currencyoffers/inappcurrencyoffers";
		var _loc4_:ASObject = {
			"action": "earn_currency",
			"product": _loc3_
		};
		if (dbFacade.facebookThirdPartyId == "0") {
			dbFacade.facebookController.queryThirdPartyId(callJSEarnCurrency);
		} else {
			callJSEarnCurrency(dbFacade, dbFacade.facebookThirdPartyId);
		}
	}
	...
	public function queryThirdPartyId(callback:ASFunction = null) {
	...
	function getNewThirdPartyId(callback:ASFunction = null) {
		if (!mDBFacade.isFacebookPlayer) {
			return;
		}
	...
	function friendsHash(isRetry:Bool = false) {
		...
		Facebook.api("/me/friends", function(param1:ASObject, param2:ASObject) {
			...
			if (ASCompat.toBool(response)) {
				fbIdArray = [];
				if (checkNullIteratee(response))
					for (_tmp_ in iterateDynamicValues(response)) {
						fbObject = _tmp_;
						fbIdArray.push(fbObject.id);
					}
				fbIdArray.sort(Reflect.compare);
				fbIdString = "";
				if (checkNullIteratee(fbIdArray))
					for (_tmp_ in fbIdArray) {
						id = _tmp_;
	...
	function loadKongregateFriendRequest(page:Int, friendIdsArray:Array<ASAny>) {
	...
	function kongregateFriendResponse(event:Event, friendIdsArray:Array<ASAny>) {
		...
		if (_loc3_.page_num < _loc3_.num_pages) {
			loadKongregateFriendRequest(ASCompat.toInt(_loc3_.page_num + 1), friendIdsArray);
		} else {
			buildKongregateFriendsHash(friendIdsArray);
		}
	}

	function buildKongregateFriendsHash(friendIdsArray:Array<ASAny>) {
		var _loc3_ = false;
		friendIdsArray.sort(Reflect.compare);
		var _loc4_ = "";
		var _loc5_:String;
		if (checkNullIteratee(friendIdsArray))
			for (_tmp_ in friendIdsArray) {
				_loc5_ = _tmp_;
		...
		if (_loc2_ != _loc6_) {
			...
			mDBFacade.dbAccountInfo.updateKongregateFriendsRPC(friendIdsArray, _loc2_, _loc3_);
		}
	}

	function feedResponseCallback(feedConstant:String, response:ASObject) {
		var _loc3_:String = null;
		if (ASCompat.toBool(response)) {
			_loc3_ = Std.string(response.post_id);
			_loc3_ = _loc3_.split("_")[0];
			if (_loc3_ == mDBFacade.facebookPlayerId) {
				mDBFacade.metrics.log("WallPostSuccess", {"feed": feedConstant});
			} else {
			...
			mDBFacade.metrics.log("WallPostCancel", {"feed": feedConstant});
		}
	}

	function giftCallback(response:ASObject, giftOfferID:UInt) {
		var _loc3_:String;
		...
		if (ASCompat.toBool(response)) {
			_loc7_ = Std.string(response.request);
			_loc9_ = ASCompat.dynamicAs(response.to, Array);
			_loc8_ = [];
			_loc5_ = [];
			...
			mDBFacade.dbAccountInfo.sendGiftData(giftOfferID, _loc5_, _loc8_);
			mDBFacade.dbAccountInfo.sendFacebookRequestData(_loc5_, _loc8_);
		}
	}

	function friendRequestCallback(response:ASObject) {
		var _loc2_:String;
		...
		var _loc3_:String = null;
		if (ASCompat.toBool(response)) {
			_loc5_ = Std.string(response.request);
			_loc6_ = ASCompat.dynamicAs(response.to, Array);
			_loc4_ = [];
	...
	public function genericFriendRequests(isRetry:Bool = false) {
		...
		if (mFriendInvitePosts.length > 0) {
			...
			friendRequests(mFriendInvitePosts[(randomPost : Int)].FeedCaption, mFriendInvitePosts[(randomPost : Int)].FeedName, "dialog",
				friendRequestCallback, filters);
		}
	}

	function dbFeedPost(gmFeedPost:GMFeedPosts, receiverId:String = "", pic:String = "", isRetry:Bool = false, replaceDict:ASObject = null) {
		var loginCallback:ASFunction;
		var feedName:String;
		var key:String;
		...
		if (ASCompat.stringAsBool(gmFeedPost.FeedName)) {
			...
			if (checkNullIteratee(replaceDict))
				for (_tmp_ in replaceDict.___keys()) {
					key = _tmp_;
	...
	public function askForKeys(feedConstant:String) {
		if (!mDBFacade.isFacebookPlayer) {
			return;
		}
		var _loc2_ = ASCompat.dynamicAs(mDBFacade.gameMaster.feedPostsByConstant.itemFor(feedConstant), gameMasterDictionary.GMFeedPosts);
		if (_loc2_ != null) {
			this.dbFeedPost(_loc2_);
		}
	}

	public function purchaseFeedPost(id:UInt) {
		this.categoryFeedPost(id, "PURCHASE");
	}

	public function chestUnlockFeedPost(id:UInt, shortItemName:String, fullItemName:String, imagePath:String) {
		this.categoryFeedPost(id, "CHEST_UNLOCK", imagePath, {
			"#ITEM": shortItemName,
			"#FULL_ITEM": fullItemName
		});
	}

	public function mapNodeDefeatedFeedPost(id:UInt, mapNodeName:String) {
		this.categoryFeedPost(id, "MAP_NODE_DEFEATED", "", {"#NODE": mapNodeName});
	}

	function categoryFeedPost(id:UInt, category:String, pic:String = "", replaceDict:ASObject = null) {
		var _loc5_ = 0;
		if (id == 0 || id == 0) {
			return;
		}
		var _loc6_ = getFeedPosts(mDBFacade, id, category);
		if (_loc6_.length > 0) {
			_loc5_ = Std.int(Math.ffloor(Math.random() * _loc6_.length));
			this.dbFeedPost(_loc6_[_loc5_], "", pic, false, replaceDict);
		}
	}
	...
	public function leaderboardFeedPostToASingleUser(facebookID:String) {
		...
		if (mLeaderBoardFeedPosts.length > 0) {
			_loc3_ = Std.int(Math.ffloor(Math.random() * mLeaderBoardFeedPosts.length));
			this.dbFeedPost(mLeaderBoardFeedPosts[_loc3_], facebookID);
		}
	}
	...
	public function updateGuestAchievement(achievementID:UInt) {
		if (!mDBFacade.isFacebookPlayer) {
			return;
		}
		if (mDBFacade.dbAccountInfo.dbAccountParams.hasAchievement(achievementID)) {
			return;
		}
		mDBFacade.dbAccountInfo.dbAccountParams.setAchievement(achievementID);
		var _loc2_ = "";
		switch (achievementID - 1) {
			case 0:
				_loc2_ = "ach1";
				_loc2_ = "ach2";
				_loc2_ = "ach3";
	...
	function achievementSuccessCallback(achievementID:String) {
		var _loc2_ = ASCompat.dynamicAs(mDBFacade.gameMaster.achievementsById.itemFor(achievementID), gameMasterDictionary.GMAchievement);
		if (_loc2_ == null) {
			return;
		}
		var _loc3_:ASObject = {
	...
	function securityErrorHandler(e:Event) {
		Logger.warn("SecurityError on scores logging: " + e.toString());
	}

	function ioErrorHandler(e:Event) {
		Logger.warn("IOError on scores logging: " + e.toString());
	}

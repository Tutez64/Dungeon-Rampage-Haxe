...
class Astar {
	...
	public function Init(dungeonFloor:DistributedDungeonFloor) {
		var _loc3_ = 0;
		var _loc2_ = 0;
		var _loc4_:Tile = null;
		mDistributedDungeonFloor = dungeonFloor;
		_loc3_ = 0;
		while (_loc3_ < 12) {
			_loc2_ = 0;
			while (_loc2_ < 12) {
	...
	public function InitTileAstarGrids(ti:UInt, tj:UInt) {
		var _loc3_ = 0;
		var _loc6_ = mDistributedDungeonFloor.tileGrid.getTileAtIndex(ti, tj);
		if (_loc6_ == null) {
			Logger.error("sent in the wrong tile position: " + ti + "," + tj);
			return;
		}
		var _loc5_ = (0 : UInt);
		var _loc7_ = (0 : UInt);
		var _loc4_ = tj * 18;
		while (_loc4_ < (tj + 1) * 18) {
			_loc3_ = ti * 18;
			while ((_loc3_ : UInt) < (ti + 1) * 18) {
	...
	public function set_HeroCollisionRadius(radius:Float):Float {
		return heroCollisionRadius = radius;
	}
	...
	public function GetAstarGrid(pos:Vector3D):UInt {
		var _loc2_ = (Std.int(pos.x / 50) : UInt);
		var _loc3_ = (Std.int(pos.y / 50) : UInt);
		return _loc2_ + _loc3_ * 216;
	}

	public function GetAstarGridCenter(pos:Vector3D):Vector3D {
		var _loc3_ = (Std.int(pos.x / 50) : UInt);
		var _loc4_ = (Std.int(pos.y / 50) : UInt);
	...
	public function GetEuclideanDistance(point1:Vector3D, point2:Vector3D):Float {
		return Math.sqrt(Math.pow(point1.x - point2.x, 2) + Math.pow(point1.y - point2.y, 2));
	}

	public function GetManhattanDistance(point1:Vector3D, point2:Vector3D):Float {
		return Math.abs(point1.x - point2.x) + Math.abs(point1.y - point2.y);
	}

	public function IsGridLegalAABB(grid:AstarGridNode):Bool {
		var collided:Bool;
	...
	public function IsGridLegalCircle(grid:AstarGridNode):Bool {
		var collided:Bool;
		var fixture:B2Fixture;
		var CollisionCallback = function(param1:B2Fixture) {
			var _loc2_ = param1.GetShape();
	...
	public function LineOfSight(point1:Vector3D, point2:Vector3D):Bool {
		var _loc3_ = NavCollider.convertToB2Vec2(point1);
		var _loc4_ = NavCollider.convertToB2Vec2(point2);
		mCollFixture = mDistributedDungeonFloor.box2DWorld.RayCastOne(_loc3_, _loc4_);
		if (mCollFixture != null) {
			Logger.info("no line of sight between " + point1 + " and " + point2);
			return false;
		}
		return true;
	}
	...
	public function Search(startPos:Vector3D, endPos:Vector3D) {
		...
		var _loc4_ = GetAstarGridCenter(startPos);
		var _loc3_ = GetAstarGridCenter(endPos);
		var _loc8_ = GetAstarGrid(startPos);
		var _loc5_ = GetAstarGrid(endPos);
		goalPoint = endPos;
	...
	public function AstarWorker(startGridIdx:UInt, goalGridIdx:UInt):Bool {
		...
		var _loc5_ = false;
		var _loc6_ = Math.NaN;
		var _loc4_ = Nodes[(startGridIdx : Int)];
		var _loc11_ = Nodes[(goalGridIdx : Int)];
		if (_loc4_ == null) {
			Logger.info("startGridIdx: " + startGridIdx + " is invalid");
			return false;
		}
		if (_loc11_ == null) {
			Logger.info("goalGridIdx: " + goalGridIdx + " is invalid");
			return false;
		}
		goalGridIdx = _loc11_.Id;
	...
	public function ReconstructPath(goalGridIdx:UInt, startGridIdx:UInt) {
		var _loc3_ = goalGridIdx;
		mAnswerPath.push(_loc3_);
		while (_loc3_ != startGridIdx) {
			_loc3_ = Nodes[(_loc3_ : Int)].Parent;
			mAnswerPath.push(_loc3_);
		}
	}

	public function DrawAstarGridsInTile(tileIdx:UInt) {
		...
		if (mDistributedDungeonFloor.debugVisualizer != null) {
			...
			_loc5_ = tileIdx % 12 * 900;
			_loc6_ = Std.int(tileIdx / 12) * 900;
			_loc7_ = Std.int(_loc5_ / 50);
			_loc8_ = Std.int(_loc6_ / 50);
			_loc2_ = _loc7_;
			while (_loc2_ < ASCompat.toNumber(_loc7_ + 18)) {
				_loc3_ = _loc8_;
				while (_loc3_ < ASCompat.toNumber(_loc8_ + 18)) {
					_loc9_ = ASCompat.toInt(_loc2_ + _loc3_ * 216);

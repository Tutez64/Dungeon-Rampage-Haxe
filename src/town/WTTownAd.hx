...
class WTTownAd implements ITownAdProvider {
	...
	public function new(dbFacade:DBFacade, townClip:MovieClip, townStateMachine:TownStateMachine) {
		mDBFacade = dbFacade;
	...
	public function assignTownClip(townClip:MovieClip) {
	...
	public function CheckForAds(responseCallback:ASFunction) {
		var _loc2_ = true;
	...
	public function WildTangentResponse(obj:ASObject) {
		Logger.debug("WT Promo Available = " + Std.string(obj.available));
		if (obj == wtResponseObject) {
			Logger.debug("WT Promo called with same obj as last");
		}
		if (mResponseCallback != null) {
			Logger.debug("WT Promo Response Callback");
			mResponseCallback(obj.available);
		}
		if (ASCompat.toBool(obj.available)) {
			wtResponseObject = obj;
			removeWildTangentAPI();
		}
	}

	public function SetResetCallback(callback:ASFunction) {
	...
	public function ShowAdPlayer() {
		...
		Logger.debug("Launching WT Ad: " + Std.string(wtResponseObject.available) + "," + Std.string(wtResponseObject.itemKey) + ","
			+ Std.string(wtResponseObject.promoName));
		mDBFacade.metrics.log("WtBrandBoostLaunch");
		mWildTangentAPI.BrandBoost.launch(wtResponseObject);
	}

	public function closedWT(obj:ASObject) {
		Logger.debug("Closed WT Ad with reason=" + Std.string(obj.reason));
		switch (obj.reason) {
			case "redeemed":
				adRedeemedWT();
	...
	public function redeemCodeWT(obj:ASObject) {
		...
		_loc2_(mDBFacade.dbAccountInfo.id, mDBFacade.dbAccountInfo.activeAvatarId, obj.vexCode, mDBFacade.validationToken, mDBFacade.demographics,
			redeemSuccessWT, redeemErrorWT);
	}

	public function redeemSuccessWT(details:ASAny) {
	...
	public function redeemErrorWT(error:Error) {

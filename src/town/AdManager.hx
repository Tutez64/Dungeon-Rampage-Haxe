...
class AdManager {
	...
	function reset(provider:UInt) {
		Logger.debug("AdManager: reset called");
		if (provider < (mProviderResponses.length:UInt)) {
			mProviderResponses[(provider : Int)] = -1;
		}
	...
	function receivedAdResponse(adProvider:UInt):ASFunction {
	...
	function ShowAdButton(adProvider:UInt) {
		if (mAdButton != null) {
			removeAdButton();
		}

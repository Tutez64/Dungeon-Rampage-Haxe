...
class CreditsTownSubState extends TownSubState {
	...
	static final Headers:Vector<String> = Vector.ofArray([
		"Dungeon Rampage\'s Revivalist",
		"Gamebreaking Studios",
		"Original Developers",
		"Special Thanks",
		"Additional Thanks",
		"Kickstarters Backers!"
	]);
	...
	public function new(dbFacade:DBFacade, townStateMachine:TownStateMachine) {
		super(dbFacade, townStateMachine, "CreditsTownSubState");
	}
	...
	override public function enterState() {
		...
		mDBFacade.menuNavigationController.pushNewLayer("CREDITS_MENU", mTownStateMachine.townHeader.determineCallback,
			mTownStateMachine.townHeader.closeButton, mTownStateMachine.townHeader.closeButton);
	}

	function textFieldToBitmap(tf:TextField):Bitmap {
		mBitMapData = new BitmapData(Std.int(tf.width * 2), Std.int(tf.height * 2), true, (0 : UInt));
		mBitMapMatrix = new Matrix();
		mBitMapMatrix.scale(2, 2);
		mBitMapData.draw(tf, mBitMapMatrix, null, null, null, true);
	...
	function resumeAutoScroll(event:MouseEvent) {
		mTween.resume();
	}

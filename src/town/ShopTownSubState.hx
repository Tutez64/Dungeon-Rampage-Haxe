...
class ShopTownSubState extends TownSubState {
	...
	public function new(dbFacade:DBFacade, townStateMachine:TownStateMachine) {
		super(dbFacade, townStateMachine, "ShopTownSubState");
	}
	...
	override public function enterState() {
		...
		mDBFacade.menuNavigationController.pushNewLayer("SHOP_MENU", mTownStateMachine.townHeader.determineCallback, mTownStateMachine.townHeader.closeButton,
			mTownStateMachine.townHeader.closeButton);
	}
	...
	public function set_startAtCategoryTab(value:String):String {
		return mStartAtCategoryTab = value;
	}
}

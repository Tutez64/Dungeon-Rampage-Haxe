...
class TownStateMachine extends StateMachine {
	...
	public function setSwfs(townAsset:SwfAsset, mapSwf:SwfAsset) {
		mTownSwfAsset = townAsset;
		mMapSwfAsset = mapSwf;
	...
	public function enterInventoryState(returnToMapStateOnClose:Bool = false, tabCategory:String = "", type:UInt = (0 : UInt), offerId:UInt = (0 : UInt),
			showEquipOption:Bool = false) {
		mInventoryState.startAtCategoryTab = tabCategory;
		mTownHeader.jumpToMapState = returnToMapStateOnClose;
		mInventoryState.setRevlealedState(type, offerId, showEquipOption);
	...
	public function enterTavernState(returnToMapStateOnClose:Bool = false) {
		mTownHeader.jumpToMapState = returnToMapStateOnClose;
	...
	public function enterShopState(returnToMapStateOnClose:Bool = false, jumpToCategoryTab:String = "") {
		mShopState.startAtCategoryTab = jumpToCategoryTab;
		mTownHeader.jumpToMapState = returnToMapStateOnClose;
	...
	public function setFriendManagementTabCategory(value:UInt) {
		mFriendManagementState.setTabCategory(value);
	}
	...
	public function getTownAsset(assetName:String):Dynamic {
		return mTownSwfAsset.getClass(assetName);
	}

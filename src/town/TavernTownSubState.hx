...
class TavernTownSubState extends TownSubState {
	...
	public function new(dbFacade:DBFacade, townStateMachine:TownStateMachine) {
		super(dbFacade, townStateMachine, "TavernTownSubState");
	...
	override public function enterState() {
		...
		mDBFacade.menuNavigationController.pushNewLayer("TAVERN_MENU", mTownStateMachine.townHeader.determineCallback,
			mTavernUI.getDefaultMenuNavigationObject());
		mTavernUI.refresh();

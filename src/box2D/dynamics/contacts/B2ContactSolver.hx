...
/*use*/ /*namespace*/ /*b2internal*/ class B2ContactSolver {
	...
	/*b2internal*/
	public var m_constraints:Vector<B2ContactConstraint> = new Vector();

	var m_constraintCount:Int = 0;

	public function new() {}

	public function Initialize(step:B2TimeStep, contacts:Vector<B2Contact>, contactCount:Int, allocator:ASAny) {
		...
		_loc6_ = 0;
		while (_loc6_ < contactCount) {
			_loc5_ = contacts[_loc6_];
			_loc9_ = _loc5_.m_fixtureA;
			...
			_loc28_ = this.m_constraints[_loc6_];
			...
			while (_loc29_ < (_loc28_.pointCount:UInt)) {
				...
				_loc32_ = _loc31_.rA.x = s_worldManifold.m_points[(_loc29_ : Int)].x - _loc15_.m_sweep.c.x;
				_loc33_ = _loc31_.rA.y = s_worldManifold.m_points[(_loc29_ : Int)].y - _loc15_.m_sweep.c.y;
				_loc34_ = _loc31_.rB.x = s_worldManifold.m_points[(_loc29_ : Int)].x - _loc16_.m_sweep.c.x;
				_loc35_ = _loc31_.rB.y = s_worldManifold.m_points[(_loc29_ : Int)].y - _loc16_.m_sweep.c.y;
				...
				_loc39_ = _loc15_.m_mass * _loc15_.m_invMass + _loc16_.m_mass * _loc16_.m_invMass;
				_loc39_ = _loc39_ + (_loc15_.m_mass * _loc15_.m_invI * _loc36_ + _loc16_.m_mass * _loc16_.m_invI * _loc37_);
		...
		while (_loc5_ < this.m_constraintCount) {
			...
			if (step.warmStarting) {
				...
				while (_loc18_ < _loc19_) {
					...
					_loc20_.normalImpulse *= step.dtRatio;
					_loc20_.tangentImpulse *= step.dtRatio;
	...
	public function SolveVelocityConstraints() {
		...
		while (_loc24_ < this.m_constraintCount) {
			_loc25_ = this.m_constraints[_loc24_];
	...
	public function SolvePositionConstraints(baumgarte:Float):Bool {
		...
		while (_loc3_ < this.m_constraintCount) {
			_loc4_ = this.m_constraints[_loc3_];
			_loc5_ = _loc4_.bodyA;
			_loc6_ = _loc4_.bodyB;
			...
			while (_loc12_ < _loc4_.pointCount) {
				...
				_loc20_ = B2Math.Clamp(baumgarte * (_loc15_ + B2Settings.b2_linearSlop), -B2Settings.b2_maxLinearCorrection, 0);

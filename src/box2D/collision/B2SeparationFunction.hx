...
class B2SeparationFunction {
	...
	public function Initialize(cache:B2SimplexCache, proxyA:B2DistanceProxy, transformA:B2Transform, proxyB:B2DistanceProxy, transformB:B2Transform) {
		...
		var _loc6_:Int = cache.count;
		B2Settings.b2Assert(0 < _loc6_ && _loc6_ < 3);
		if (_loc6_ == 1) {
			this.m_type = e_points;
			_loc7_ = this.m_proxyA.GetVertex(cache.indexA[0]);
			_loc10_ = this.m_proxyB.GetVertex(cache.indexB[0]);
			_loc20_ = _loc7_;
			_loc19_ = transformA.R;
			_loc13_ = transformA.position.x + (_loc19_.col1.x * _loc20_.x + _loc19_.col2.x * _loc20_.y);
			_loc14_ = transformA.position.y + (_loc19_.col1.y * _loc20_.x + _loc19_.col2.y * _loc20_.y);
			_loc20_ = _loc10_;
			_loc19_ = transformB.R;
			_loc15_ = transformB.position.x + (_loc19_.col1.x * _loc20_.x + _loc19_.col2.x * _loc20_.y);
			_loc16_ = transformB.position.y + (_loc19_.col1.y * _loc20_.x + _loc19_.col2.y * _loc20_.y);
			this.m_axis.x = _loc15_ - _loc13_;
			this.m_axis.y = _loc16_ - _loc14_;
			this.m_axis.Normalize();
		} else if (cache.indexB[0] == cache.indexB[1]) {
			this.m_type = e_faceA;
			_loc8_ = this.m_proxyA.GetVertex(cache.indexA[0]);
			_loc9_ = this.m_proxyA.GetVertex(cache.indexA[1]);
			_loc10_ = this.m_proxyB.GetVertex(cache.indexB[0]);
			...
			this.m_type = e_faceB;
			_loc11_ = this.m_proxyB.GetVertex(cache.indexB[0]);
			_loc12_ = this.m_proxyB.GetVertex(cache.indexB[1]);
			_loc7_ = this.m_proxyA.GetVertex(cache.indexA[0]);
			...
			_loc11_ = this.m_proxyB.GetVertex(cache.indexB[0]);
			_loc12_ = this.m_proxyB.GetVertex(cache.indexB[1]);
			_loc23_ = B2Math.MulX(transformA, _loc7_);
			_loc24_ = B2Math.MulMV(transformA.R, B2Math.SubtractVV(_loc9_, _loc8_));
			_loc25_ = B2Math.MulX(transformB, _loc10_);
			_loc26_ = B2Math.MulMV(transformB.R, B2Math.SubtractVV(_loc12_, _loc11_));
	...
	public function Evaluate(transformA:B2Transform, transformB:B2Transform):Float {
		...
		switch (this.m_type) {
			case e_points:
				_loc3_ = B2Math.MulTMV(transformA.R, this.m_axis);
				_loc4_ = B2Math.MulTMV(transformB.R, this.m_axis.GetNegative());
				_loc5_ = this.m_proxyA.GetSupportVertex(_loc3_);
				_loc6_ = this.m_proxyB.GetSupportVertex(_loc4_);
				_loc7_ = B2Math.MulX(transformA, _loc5_);
				_loc8_ = B2Math.MulX(transformB, _loc6_);
				return (_loc8_.x - _loc7_.x) * this.m_axis.x + (_loc8_.y - _loc7_.y) * this.m_axis.y;
			case e_faceA:
				_loc10_ = B2Math.MulMV(transformA.R, this.m_axis);
				_loc7_ = B2Math.MulX(transformA, this.m_localPoint);
				_loc4_ = B2Math.MulTMV(transformB.R, _loc10_.GetNegative());
				_loc6_ = this.m_proxyB.GetSupportVertex(_loc4_);
				_loc8_ = B2Math.MulX(transformB, _loc6_);
				return (_loc8_.x - _loc7_.x) * _loc10_.x + (_loc8_.y - _loc7_.y) * _loc10_.y;
			case e_faceB:
				_loc10_ = B2Math.MulMV(transformB.R, this.m_axis);
				_loc8_ = B2Math.MulX(transformB, this.m_localPoint);
				_loc3_ = B2Math.MulTMV(transformA.R, _loc10_.GetNegative());
				_loc5_ = this.m_proxyA.GetSupportVertex(_loc3_);
				_loc7_ = B2Math.MulX(transformA, _loc5_);

...
class B2Simplex {
	...
	public function ReadCache(cache:B2SimplexCache, proxyA:B2DistanceProxy, transformA:B2Transform, proxyB:B2DistanceProxy, transformB:B2Transform) {
		...
		this.m_count = (cache.count : Int);
		var _loc8_ = this.m_vertices;
		var _loc9_ = 0;
		while (_loc9_ < this.m_count) {
			_loc10_ = _loc8_[_loc9_];
			_loc10_.indexA = cache.indexA[_loc9_];
			_loc10_.indexB = cache.indexB[_loc9_];
			_loc6_ = proxyA.GetVertex(_loc10_.indexA);
			_loc7_ = proxyB.GetVertex(_loc10_.indexB);
			_loc10_.wA = B2Math.MulX(transformA, _loc6_);
			_loc10_.wB = B2Math.MulX(transformB, _loc7_);
			_loc10_.w = B2Math.SubtractVV(_loc10_.wB, _loc10_.wA);
			_loc10_.a = 0;
			_loc9_ = ASCompat.toInt(_loc9_) + 1;
		}
		if (this.m_count > 1) {
			_loc11_ = cache.metric;
		...
		if (this.m_count == 0) {
			...
			_loc10_.indexA = 0;
			_loc10_.indexB = 0;
			_loc6_ = proxyA.GetVertex(0);
			_loc7_ = proxyB.GetVertex(0);
			_loc10_.wA = B2Math.MulX(transformA, _loc6_);
			_loc10_.wB = B2Math.MulX(transformB, _loc7_);
			_loc10_.w = B2Math.SubtractVV(_loc10_.wB, _loc10_.wA);
			this.m_count = 1;
	...
	public function WriteCache(cache:B2SimplexCache) {
		cache.metric = this.GetMetric();
		cache.count = (this.m_count : UInt);
		var _loc2_ = this.m_vertices;
		var _loc3_ = 0;
		while (_loc3_ < this.m_count) {
			cache.indexA[_loc3_] = _loc2_[_loc3_].indexA;
			cache.indexB[_loc3_] = _loc2_[_loc3_].indexB;
	...
	public function GetWitnessPoints(pA:B2Vec2, pB:B2Vec2) {
		switch (this.m_count) {
			case 0:
				B2Settings.b2Assert(false);

...
class DBInventoryInfo {
	...
	public function new(dbFacade:DBFacade, responseCallback:ASFunction, parseErrorCallback:ASFunction) {
		mDBFacade = dbFacade;
		mAvatars = new Map();
		mItems = new Map();
		...
		mParseErrorCallback = parseErrorCallback;
	...
	public function getStackableByStackId(stackIndex:Int):StackableInfo {
		return ASCompat.dynamicAs(mStackablesByStackableId.itemFor(stackIndex), account.StackableInfo);
	}

	public function findHighestBoosterXP():BoosterInfo {
		var _loc4_:BoosterInfo = null;
		...
		if (checkNullIteratee(_loc2_))
			for (_tmp_ in 0..._loc2_.length) {
				_loc3_ = Std.string(_tmp_);
				_loc5_ = ASCompat.toInt((_loc2_ : ASAny)[ASCompat.toInt(_loc3_)]);
	...
	public function timestampBooster(stackId:Int):String {
		var _loc2_:BoosterInfo = null;
		if (mBoostersByStackableId.hasKey(stackId)) {
			_loc2_ = ASCompat.dynamicAs(mBoostersByStackableId.itemFor(stackId), account.BoosterInfo);
			if (_loc2_ != null) {
	...
	public function timeTillBoosterExpire(stackId:Int):Int {
		var _loc3_ = ASCompat.dynamicAs(mBoostersByStackableId.itemFor(stackId), account.BoosterInfo);
	...
	public function timeTillNextBoosterExpire():Int {
		...
		if (checkNullIteratee(_loc3_))
			for (_tmp_ in 0..._loc3_.length) {
				_loc4_ = Std.string(_tmp_);
				_loc6_ = ASCompat.toInt((_loc3_ : ASAny)[ASCompat.toInt(_loc4_)]);
	...
	public function dateBooster(stackId:Int):Date {
		var _loc2_ = GameClock.getWebServerDate();
		var _loc3_ = timestampBooster(stackId);
		if (_loc3_ != "") {
	...
	public function get_unequippedWeaponCount():UInt {
		...
		return (_loc1_ + mChests.length : UInt);
	}
	...
	public function getStackCount(stackableId:UInt):UInt {
		var _loc2_ = ASCompat.dynamicAs(mStackablesByStackableId.itemFor(stackableId), account.StackableInfo);
		return (ASCompat.toInt(_loc2_ != null ? _loc2_.count : (0 : UInt)) : UInt);
	}

	public function getAvatarInfoForAvatarInstanceId(instanceId:UInt):AvatarInfo {
		return ASCompat.dynamicAs(mAvatars.itemFor(instanceId), account.AvatarInfo);
	}

	public function getAvatarInfoForHeroType(heroType:UInt):AvatarInfo {
	...
	function modifiersMatch(details:GMOfferDetail, itemInfo:ItemInfo):Bool {
		var _loc3_ = (0 : UInt);
		if (ASCompat.stringAsBool(details.Modifier1)) {
			_loc3_++;
		}
		if (ASCompat.stringAsBool(details.Modifier2)) {
			_loc3_++;
		}
		if (ASCompat.stringAsBool(details.Modifier3)) {
			_loc3_++;
		}
		if (_loc3_ != (itemInfo.modifiers.length : UInt)) {
			return false;
		}
		if (ASCompat.stringAsBool(details.Modifier1) && !itemInfo.hasModifier(details.Modifier1)) {
			return false;
		}
		if (ASCompat.stringAsBool(details.Modifier2) && !itemInfo.hasModifier(details.Modifier2)) {
			return false;
		}
		if (ASCompat.stringAsBool(details.Modifier3) && !itemInfo.hasModifier(details.Modifier3)) {
			return false;
		}
		return true;
	}

	public function ownsExactWeapon(details:GMOfferDetail):Bool {
		var _loc2_:IMapIterator = null;
		var _loc3_:ItemInfo = null;
		_loc2_ = ASCompat.reinterpretAs(mItems.iterator(), IMapIterator);
		while (_loc2_.hasNext()) {
			_loc3_ = ASCompat.dynamicAs(_loc2_.next(), account.ItemInfo);
			if (details.WeaponId == _loc3_.gmId && details.WeaponPower == _loc3_.power && this.modifiersMatch(details, _loc3_)) {
				return true;
			}
		}
		return false;
	...
	public function ownsItem(infoId:UInt):Bool {
		...
		while (_loc5_.hasNext()) {
			...
			if (infoId == _loc6_.gmId) {
				return true;
			}
		}
		_loc7_ = ASCompat.dynamicAs(mStackablesByStackableId.itemFor(infoId), account.StackableInfo);
		...
		while (_loc5_.hasNext()) {
			...
			if (infoId == _loc3_.skinType) {
				return true;
			}
		}
		if (mDBFacade.gameMaster.isSkinTypeADefaultSkin(infoId)) {
			return true;
		}
		return false;
	}

	public function getEquipedItemsOnAvatar(avatarInstanceId:UInt):Vector<ItemInfo> {
		var _loc3_:IMapIterator = null;
		...
		while (_loc3_.hasNext()) {
			...
			if (_loc4_.avatarId == avatarInstanceId) {
				_loc2_.push(_loc4_);
			}
		}
		return _loc2_;
	...
	public function getEquipedPetsOnAvatar(avatarInstanceId:UInt):Vector<PetInfo> {
		...
		while (_loc4_.hasNext()) {
			...
			if (_loc3_.EquippedHero == avatarInstanceId) {
				_loc2_.push(_loc3_);
			}
		}
		return _loc2_;
	...
	public function getEquipedConsumablesOnAvatar(avatarInfo:AvatarInfo):Vector<StackableInfo> {
		var _loc4_:ASAny = null;
		var _loc5_:StackableInfo = null;
		var _loc2_:GMStackable = null;
		var _loc3_ = new Vector<StackableInfo>();
		if (avatarInfo.consumable1Id > 0 && avatarInfo.consumable1Count > 0) {
			_loc2_ = ASCompat.dynamicAs(mDBFacade.gameMaster.stackableById.itemFor(avatarInfo.consumable1Id), gameMasterDictionary.GMStackable);
			_loc5_ = new StackableInfo(mDBFacade, null, _loc2_);
			_loc5_.setPropertiesAsConsumable(_loc2_.Id, (0 : UInt), avatarInfo.consumable1Count);
			_loc3_.push(_loc5_);
		}
		if (avatarInfo.consumable2Id > 0 && avatarInfo.consumable2Count > 0) {
			_loc2_ = ASCompat.dynamicAs(mDBFacade.gameMaster.stackableById.itemFor(avatarInfo.consumable2Id), gameMasterDictionary.GMStackable);
			_loc5_ = new StackableInfo(mDBFacade, null,
				ASCompat.dynamicAs(mDBFacade.gameMaster.stackableById.itemFor(avatarInfo.consumable2Id), gameMasterDictionary.GMStackable));
			_loc5_.setPropertiesAsConsumable(_loc2_.Id, (1 : UInt), avatarInfo.consumable2Count);
			_loc3_.push(_loc5_);
		}
		return _loc3_;
	}
	...
	public function parseJson(accountJson:ASObject) {
		if (ASCompat.toBool(accountJson.account_avatars)) {
			parseAvatars(accountJson.account_avatars);
		}
		if (ASCompat.toBool(accountJson.account_items)) {
			parseItems(accountJson.account_items);
		}
		if (ASCompat.toBool(accountJson.account_chests)) {
			parseChests(accountJson.account_chests);
		}
		if (ASCompat.toBool(accountJson)) {
			parseKeys(accountJson);
		}
		if (ASCompat.toBool(accountJson.account_stackables)) {
			parseStackables(accountJson.account_stackables);
		}
		if (ASCompat.toBool(accountJson.buckets_weapon)) {
			mStorageLimitWeapon = (ASCompat.toInt(accountJson.buckets_weapon) : UInt);
		}
		if (ASCompat.toBool(accountJson.buckets_other)) {
			mStorageLimitOther = (ASCompat.toInt(accountJson.buckets_other) : UInt);
		}
		if (ASCompat.toBool(accountJson.account_pets)) {
			parsePetInfo(accountJson.account_pets);
		}
		if (ASCompat.toBool(accountJson.account_skins)) {
			parseSkinInfo(accountJson.account_skins);
		}
		if (ASCompat.toBool(accountJson.account_boosters)) {
			parseBoosters(accountJson.account_boosters);
		}
		mFirstParse = false;
		mTotalHeroesOwned = mAvatars.size;
	}

	function parseAvatars(avatarsJson:ASObject) {
		var _loc2_:AvatarInfo = null;
		var _loc4_ = 0;
		mAvatars.clear();
		var _loc3_:Array<ASAny> = ASCompat.dynamicAs(avatarsJson, Array);
		_loc4_ = 0;
	...
	function parseItems(itemsJson:ASObject) {
		var _loc4_:ItemInfo = null;
		var _loc2_ = 0;
		var _loc3_ = mItems.keysToArray();
		mItems.clear();
		var _loc5_:Array<ASAny> = ASCompat.dynamicAs(itemsJson, Array);
		_loc2_ = 0;
	...
	function parseChests(chestsJson:ASObject) {
		...
		var _loc2_:Array<ASAny> = ASCompat.dynamicAs(chestsJson, Array);
		_loc7_ = 0;
		while (_loc7_ < _loc2_.length) {
			...
			if (!mFirstParse) {
				if (checkNullIteratee(_loc3_))
					for (_tmp_ in _loc3_) {
						_loc4_ = _tmp_;
						if (_loc4_.databaseId == ASCompat.toNumberField(_loc2_[_loc7_], "id")) {
							_loc5_ = true;
							break;
						}
					}
	...
	function parseKeys(accountsJson:ASObject) {
		...
		while (_loc4_ < _loc3_.length) {
			...
			switch (_loc4_) {
				case 0:
					_loc2_ = ASCompat.toInt(accountsJson.basic_keys);

				case 1:
					_loc2_ = ASCompat.toInt(accountsJson.uncommon_keys);

				case 2:
					_loc2_ = ASCompat.toInt(accountsJson.rare_keys);

				case 3:
					_loc2_ = ASCompat.toInt(accountsJson.legendary_keys);
			}
			_loc6_ = new KeyInfo(ASCompat.dynamicAs(mDBFacade.gameMaster.keysByOfferId.itemFor(_loc5_), gameMasterDictionary.GMKey),
				ASCompat.dynamicAs(mDBFacade.gameMaster.offerById.itemFor(_loc5_), gameMasterDictionary.GMOffer), (_loc2_ : UInt));
			mKeys.push(_loc6_);
			_loc4_ = ASCompat.toInt(_loc4_) + 1;
		}
	}

	function parseStackables(stackablesJson:ASObject) {
		var _loc5_:StackableInfo = null;
		var _loc4_ = 0;
		var _loc2_ = mStackablesByDatabaseId.keysToArray();
		mStackablesByDatabaseId.clear();
		mStackablesByStackableId.clear();
		var _loc3_:Array<ASAny> = ASCompat.dynamicAs(stackablesJson, Array);
		_loc4_ = 0;
		while (_loc4_ < _loc3_.length) {
	...
	function parseBoosters(boostersJson:ASObject) {
		var _loc2_:BoosterInfo = null;
		var _loc3_ = 0;
		var _loc5_ = mBoostersByDatabaseId.keysToArray();
		mBoostersByDatabaseId.clear();
		mBoostersByStackableId.clear();
		var _loc4_:Array<ASAny> = ASCompat.dynamicAs(boostersJson, Array);
		_loc3_ = 0;
	...
	function parsePetInfo(petsJson:ASObject) {
		var _loc3_:PetInfo = null;
		var _loc4_ = 0;
		var _loc5_ = mPets.keysToArray();
		mPets.clear();
		var _loc2_:Array<ASAny> = ASCompat.dynamicAs(petsJson, Array);
		_loc4_ = 0;
	...
	function parseSkinInfo(skinsJson:ASObject) {
		var _loc3_:SkinInfo = null;
		var _loc4_ = 0;
		mSkins.clear();
		var _loc2_:Array<ASAny> = ASCompat.dynamicAs(skinsJson, Array);
		_loc4_ = 0;
		while (_loc4_ < _loc2_.length) {
	...
	public function openChest(chestInstanceId:UInt, forHeroId:UInt, forHeroSkinId:UInt, responseFinishedCallback:ASFunction = null,
	...
	public function dropChest(chestInstanceId:UInt, responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
	...
	public function canThisAvatarEquipThisItem(avatarInfo:AvatarInfo, itemInfo:ItemInfo):Bool {
		if (avatarInfo == null) {
			return false;
		}
		if (itemInfo == null) {
		...
		if (!canAvatarEquipThisMasterType(avatarInfo, itemInfo.gmWeaponItem.MasterType)) {
			return false;
		}
		if (avatarInfo.level < itemInfo.requiredLevel) {
			return false;
		}
		return true;
	}

	public function canAvatarEquipThisMasterType(avatarInfo:AvatarInfo, masterType:String):Bool {
		return avatarInfo.gmHero.AllowedWeapons.hasKey(masterType);
	}

	public function recalculateWeaponPowers(responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
	...
	public function equipItemOnAvatar(avatarInstanceId:UInt, itemInstanceId:UInt, equipSlot:UInt, responseFinishedCallback:ASFunction = null,
			errorCallback:ASFunction = null) {
	...
	public function unequipItemOffAvatar(itemInfo:InventoryBaseInfo, responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
		var rpcFunc:ASFunction;
	...
	public function equipConsumableOnAvatar(avatarInstanceId:UInt, stackableId:UInt, equipSlot:UInt, swapping:Bool = false,
			responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
	...
	public function unequipConsumableOffAvatar(avatarInstanceId:UInt, stackableId:UInt, equipSlot:UInt, responseFinishedCallback:ASFunction = null,
			errorCallback:ASFunction = null) {
	...
	public function canEquipThisConsumable(avatarInfo:AvatarInfo, equipSlot:UInt, consumableId:UInt):Bool {
		var _loc4_ = true;
		if (equipSlot == 0) {
			if (avatarInfo.consumable2Id > 0 && avatarInfo.consumable2Count > 0) {
				_loc4_ = avatarInfo.consumable2Id != consumableId;
			}
		} else if (equipSlot == 1) {
			if (avatarInfo.consumable1Id > 0 && avatarInfo.consumable1Count > 0) {
				_loc4_ = avatarInfo.consumable1Id != consumableId;
			}
		}
		return _loc4_;
	}

	public function equipPetOnAvatar(avatarInstanceId:UInt, petId:UInt, responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
	...
	public function unequipPet(petInfo:PetInfo, responseFinishedCallback:ASFunction = null, errorCallback:ASFunction = null) {
		var rpcFunc:ASFunction;
	...
	public function isEquippableByAnyOwnedAvatar(itemInfo:ItemInfo):Bool {
		var _loc3_:IMapIterator = null;
		var _loc2_:AvatarInfo = null;
		_loc3_ = ASCompat.reinterpretAs(mAvatars.iterator(), IMapIterator);
		while (_loc3_.hasNext()) {
			_loc2_ = ASCompat.dynamicAs(_loc3_.next(), account.AvatarInfo);
			if (this.canThisAvatarEquipThisItem(_loc2_, itemInfo)) {
				return true;
			}
		}
		return false;
	...
	public function getSkinsForHero(gmHero:GMHero, offerBySkinId:Map):Vector<GMSkin> {
		var skin:GMSkin;
		var offer:GMOffer;
	...
	public function isSkinExpired(skin:GMSkin, offer:GMOffer):Bool {
		...
		var _loc5_:Float = 0;
		if (offer != null) {
			_loc4_ = offer.VisibleDate != null || offer.StartDate != null;
			if (_loc4_) {
				_loc3_ = offer.VisibleDate != null ? offer.VisibleDate.getTime() : offer.StartDate.getTime();
			}
			_loc6_ = offer.EndDate != null || offer.SoldOutDate != null;
			if (_loc6_) {
				_loc5_ = offer.SoldOutDate != null ? offer.SoldOutDate.getTime() : offer.EndDate.getTime();
			}
		}
		if (doesPlayerOwnSkin(skin.Id) || (offer != null && !_loc4_ || _loc3_ <= _loc7_ && (!_loc6_ || _loc5_ > _loc7_))) {
			return false;
		}
		return true;
	}

	public function doesPlayerOwnSkin(skinId:UInt):Bool {
		return mSkins.hasKey(skinId) || mDBFacade.gameMaster.isSkinTypeADefaultSkin(skinId);
	}

	public function getSkinInfo(skinId:UInt):SkinInfo {
		if (mSkins.hasKey(skinId)) {
			return ASCompat.dynamicAs(mSkins.itemFor(skinId), actor.player.SkinInfo);
		}
		return null;
	}

	public function getDefaultSkinForHero(gmHero:GMHero):GMSkin {
		var _loc2_ = mDBFacade.gameMaster.getSkinByConstant(gmHero.DefaultSkin);
		if (_loc2_ == null) {
			Logger.error("Unable to find default skin for hero type: " + gmHero.Id);
		}
		return _loc2_;
	}

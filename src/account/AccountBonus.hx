...
class AccountBonus {
	...
	public function setXPBonusStuff(event:XPBonusEvent) {
		mXPBonusActive = event.isActive;
		mXPBonusMultiplier = event.xpMultiplier;
		mXPBonusText = event.xpMultiplierBonusTextForHUD;
		mUINewsFeedXPBonusText = event.xpMultiplierBonusTextForNewsFeed;
		mEventComponent.dispatchEvent(new Event("BONUS_XP_CHANGED_EVENT"));
		if (mXPBonusMultiplier > 1) {
			mEventComponent.dispatchEvent(new GenericNewsFeedEvent("GENERIC_NEWS_FEED_MESSAGE_EVENT", mUINewsFeedXPBonusText,
				DBFacade.buildFullDownloadPath("Resources/Art2D/UI/db_UI_town.swf"), "UI_alert_doublexp"));
		}
	}

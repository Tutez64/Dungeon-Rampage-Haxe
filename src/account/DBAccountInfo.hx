...
class DBAccountInfo {
	...
	public function alterAttribute(name:String, value:String) {
		var rpcSuccessCallback:ASFunction;
	...
	public function getAttribute(name:String):String {
		return mAttributes.itemFor(name);
	}
	...
	function set_hudStyle(value:UInt):UInt {
		return mHudStyle = value;
	}

	public function acceptGift(requestId:String, clearGiftCallback:ASFunction, successCallback:ASFunction = null) {
	...
	public function acceptAllGifts(clearGiftCallback:ASFunction, successCallback:ASFunction = null) {
	...
	public function declineGift(requestId:String, successCallback:ASFunction = null) {
	...
	public function sendGiftData(offerId:UInt, requestIds:Array<ASAny>, toIds:Array<ASAny>, successCallback:ASFunction = null) {
		...
		rpcFunc(mDBFacade.dbAccountInfo.id, offerId, mDBFacade.dbConfigManager.networkId, requestIds, toIds, mDBFacade.validationToken, rpcSuccessCallback,
			parseError);
	}

	public function sendFacebookRequestData(requestIds:Array<ASAny>, toIds:Array<ASAny>, successCallback:ASFunction = null) {
		var _loc4_ = JSONRPCService.getFunction("AppRequests", mDBFacade.rpcRoot + "facebookrequests");
		_loc4_(mDBFacade.dbAccountInfo.id, requestIds, toIds, mDBFacade.validationToken);
	}

	public function getGiftData(successCallback:ASFunction = null) {
	...
	function parseGiftData(gifts:ASAny) {
		var _loc3_:Array<ASAny> = null;
		var _loc2_:GiftInfo = null;
		var _loc4_ = 0;
		if (ASCompat.toBool(gifts)) {
			if (ASCompat.toBool(gifts.excludeIds)) {
				mGiftExcludeIds = ASCompat.dynamicAs(gifts.excludeIds, Array);
			}
			if (!ASCompat.toBool(gifts.gifts)) {
				return;
			}
			_loc3_ = ASCompat.dynamicAs(gifts.gifts, Array);
			_loc4_ = 0;
	...
	public function addFriendCallback(friends:ASAny) {
		var _loc2_:Array<ASAny> = null;
		var _loc5_:FriendInfo = null;
		var _loc4_ = 0;
		var _loc3_ = 0;
		if (ASCompat.toBool(friends)) {
			_loc2_ = ASCompat.dynamicAs(friends, Array);
			_loc4_ = 0;
	...
	public function removeFriendCallback(friends:ASAny) {
		var _loc2_:Array<ASAny> = null;
	...
	public function isFriend(personId:UInt):Bool {
		if (personId == 0) {
			return false;
		}
		return mFriendsInfo.hasKey(personId);
	}

	public function getFriendData(successCallback:ASFunction) {
	...
	public function refreshFriendData(friends:ASAny) {
		parseFriendData(friends);
	}

	public function getIgnoredFriendData(successCallback:ASFunction = null) {
		var rpcFunc:ASFunction;
		var rpcSuccessCallback:ASFunction;
	...
	function parseIgnoredFriendData(ignoredFriends:ASAny) {
		var _loc2_:Array<ASAny> = null;
		var _loc5_:FriendInfo = null;
		var _loc4_ = 0;
		var _loc3_ = 0;
		if (ASCompat.toBool(ignoredFriends)) {
			_loc2_ = ASCompat.dynamicAs(ignoredFriends, Array);
			_loc4_ = 0;
			while (_loc4_ < _loc2_.length) {
				_loc3_ = (ASCompat.asUint(_loc2_[_loc4_].account_id) : Int);
				if (!mFriendsInfo.hasKey(_loc3_)) {
	...
	function friendlistUpdate(event:FriendStatusEvent) {
	...
	function parseFriendData(friends:ASAny, purge:Bool = false) {
	...
	public function addAchievement(achievementId:String, achievementURL:String, achievementCallback:ASFunction) {
	...
	public function setPresenceTask(location:String) {
	...
	function setOnlinePresence(gameClock:GameClock) {
		var _loc6_ = (gameClock.realTime : UInt);
		var _loc4_ = gameClock.frame;
	...
	public function getFacebookIdRPC(remoteId:UInt, successCallback:ASFunction) {
	...
	function set_giftExcludeIds(excludeIds:Array<ASAny>):Array<ASAny> {
		return mGiftExcludeIds = excludeIds;
	}
	...
	public function set_account_flags(val:UInt):UInt {
		return mAccountFlags = val;
	}
	...
	function parseAttributes(attributeArray:Array<ASAny>) {
		mAttributes.clear();
		var _loc2_:ASObject;
		if (checkNullIteratee(attributeArray))
			for (_tmp_ in attributeArray) {
				_loc2_ = _tmp_;
				mAttributes.add(_loc2_.name, _loc2_.value);
			}
	}

	function parseCurrency(accountJson:ASObject) {
		mPremiumCurrency = (ASCompat.toInt(accountJson.premium_currency) : UInt);
		mBasicCurrency = (ASCompat.toInt(accountJson.basic_currency) : UInt);
		mTrophyCount = (ASCompat.toInt(accountJson.trophies) : UInt);
	...
	public function changeActiveAvatarRPC(heroType:UInt) {
	...
	public function getAllMapnodeScoresRPC(requestor:UInt) {
	...
	public function updateFacebookFriendsRPC(friends:Array<ASAny>, newFriendsHash:String, cascade:Bool) {
	...
	public function updateDRFriendsRPC(friends:Array<ASAny>, newFriendsHash:String) {
		var _loc3_ = JSONRPCService.getFunction("inviteNewDRFriends", mDBFacade.rpcRoot + "leaderboard");
		mCurrentFriendsHash = newFriendsHash;
		_loc3_(mDBFacade.dbAccountInfo.id, mDBFacade.validationToken, friends, newFriendsHash, mDBFacade.dbAccountInfo.name, mDBFacade.dbAccountInfo.trophies,
			mDBFacade.dbAccountInfo.activeAvatarSkinId, mDBFacade.dbAccountInfo.facebookId);
	}

	public function updateKongregateFriendsRPC(friends:Array<ASAny>, newFriendsHash:String, cascade:Bool) {
	...
	public function parseFriendResponse(details:ASAny, callRefreshOnceDone:Bool = true) {
		if (ASCompat.toBool(details.friends_hash)) {
			mCurrentFriendsHash = ASCompat.asString(details.friends_hash);
		}
	}

	public function parseScoreResponse(details:ASObject, callRefreshOnceDone:Bool = true) {
		if (details == null) {
			Logger.error("Got empty array on parseScoreResponse");
			return;
		}
		var _loc4_ = new II_AccountTopScoreInfo(details.top_scores);
		...
		while (_loc3_.hasNext()) {
			...
			if (ASCompat.toNumberField(_loc3_.current, "id") == mDBFacade.accountId) {
				_loc3_.current.parseIIAvatarScoresData(details.avatar_scores);
			}
		}
	}

	public function parseResponse(details:ASAny, callRefreshOnceDone:Bool = true) {
		if (details == null || ASCompat.toNumberField(details, "length") <= 0) {
			Logger.error("Got empty array on parseResponse");
			return;
		}
		mCompletedMapnodeMask.init(details.completed_mapnode_mask);
		mAccountFlags = ASCompat.asUint(details.account_flags);
		var _loc3_ = ASCompat.toInt(details.active_avatar);
		mInventory.parseJson(details);
		...
		parseCurrency(details);
		if (ASCompat.toBool(details.account_attributes)) {
			parseAttributes(ASCompat.dynamicAs(details.account_attributes, Array));
		}
		mId = (ASCompat.toInt(details.id) : UInt);
		mName = details.name;
		mInventoryLimitWeapons = (ASCompat.toInt(details.buckets_weapon) : UInt);
		mInventoryLimitOther = (ASCompat.toInt(details.buckets_other) : UInt);
		mAccountCreatedDate = ASCompat.asString(details.created);
		mDungeonsCompleted = (ASCompat.toInt(details.completed_dungeons) : UInt);
		mDBAccountParams = new DBAccountParams(mDBFacade, this);
		if (callRefreshOnceDone) {
	...
	function killErrorPopUp(gameClock:GameClock) {
		if (mErrorPopUp != null) {
			mErrorPopUp.destroy();
			mErrorPopUp = null;
		}

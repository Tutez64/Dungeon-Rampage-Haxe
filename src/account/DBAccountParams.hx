...
class DBAccountParams {
	...
	public function new(dbFacade:DBFacade, accountInfo:DBAccountInfo) {
		mDBFacade = dbFacade;
		mDBAccountInfo = accountInfo;
	...
	public function hasParam(paramNum:UInt):Bool {
		var _loc2_ = ((staticDBAccountParamsHelper.current_account_flags : Int) & 1 << (paramNum : Int)) != 0;
		return ((staticDBAccountParamsHelper.current_account_flags : Int) & 1 << (paramNum : Int)) != 0;
	}

	public function setParam(paramNum:UInt, updateImmediately:Bool = true) {
		staticDBAccountParamsHelper.current_account_flags = ((staticDBAccountParamsHelper.current_account_flags | (1 << (paramNum : Int) : UInt):UInt) : UInt);
	...
	public function flushParams() {
		...
		rpcFunc(mDBFacade.dbAccountInfo.id, mDBFacade.validationToken, staticDBAccountParamsHelper.current_account_flags, rpcSuccessCallback,
			rpcFailureCallback);
	...
	public function hasAchievement(achievementNum:UInt):Bool {
		switch (achievementNum - 1) {
			case 0:
	...
	public function setAchievement(achievementNum:UInt) {
		switch (achievementNum - 1) {
			case 0:

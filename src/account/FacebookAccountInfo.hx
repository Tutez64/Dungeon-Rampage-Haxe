...
class FacebookAccountInfo {
	...
	function loadedFacebookAccountInfo(e:Event) {
	...
	function popupError(event:Event) {
		Logger.warn("FacebookAccountInfo Error: " + event.toString() + " " + Std.string(event.target));
	...
	public function loadFriends(callback:ASFunction = null) {
	...
	function loadedFriends(e:Event) {
		var _loc3_ = cast(e.target, URLLoader);
		var _loc2_:ASObject = com.adobe.serialization.json.JSON.decode(_loc3_.data);
	...
	public function loadFriendProfilePic(friendId:String, type:String = "square"):DisplayObject {
		var _loc4_ = new Loader();
		var _loc3_ = "https://graph.facebook.com/" + friendId + "/picture?type=" + type;

...
class Floor extends GameObject {
	...
	public function set_activeDungeonModifiers(val:Vector<DungeonModifier>):Vector<DungeonModifier> {
		var _loc2_ = 0;
		_loc2_ = 0;
		while (_loc2_ < val.length) {
			mActiveDungeonModifiers.push(new DungeonModifierHelper(val[_loc2_].id, val[_loc2_].new_this_floor == 1, mDBFacade));
	...
	public function floorEnding(timeUntilTransition:UInt) {
		mFloorEndingGui.floorEnding(timeUntilTransition);
	}

	public function floorFailing(timeUntilTransition:UInt) {
		mFloorEndingGui.floorFailing(timeUntilTransition);
	}
	...
	function update(gameClock:GameClock) {
		mB2World.Step(gameClock.tickLength, 8, 3);
	...
	public function SetParentArea(parentArea:Area) {
	...
	public function addCollectedTreasure(type:UInt) {
		mParentArea.addCollectedTreasure(type);
	}
	...
	public function addCollectedExp(amount:UInt) {
		mParentArea.addCollectedExp(amount);
	}

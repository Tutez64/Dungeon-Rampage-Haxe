...
class LoadingState extends State {
	...
	public function new(dbFacade:DBFacade, finishedCallback:ASFunction = null) {
		super("LoadingState", finishedCallback);
		mDBFacade = dbFacade;
	}
	...
	function configReady(event:ServiceDiscoveryReadyEvent) {
		Logger.info("LoadingState configReady");
		mDBFacade.loadingBarTick();
		if (event.serviceDiscoveryResult == null) {
			mDBFacade.mainStateMachine.enterSocketErrorState((event.serviceDiscoveryErrorCode : UInt), event.serviceDiscoveryErrorText);
			return;
		}
		mDBFacade.applyServiceDiscoveryResult(event.serviceDiscoveryResult);
		mConfigLoaded = true;
	...
	function matchMakerReady(event:MatchMakerLoadedEvent) {
	...
	function managersLoaded(event:ManagersLoadedEvent) {
	...
	function accountLoaded(dbAccountLoadedEvent:DBAccountLoadedEvent) {
		...
		mDBFacade.loadingBarTick();
		mAccountLoaded = true;
		mDBAccountInfo = dbAccountLoadedEvent.dbAccountInfo;
		checkIfLoadingFinished();
	}

	function checkIfLoadingFinished() {
		Logger.info("--->checkIfLoadingFinished  mConfigLoaded:" + Std.string(mConfigLoaded) + " mManagersLoaded:" + Std.string(mManagersLoaded)
			+ " mTransitionsLoaded:" + Std.string(mTransitionsLoaded) + " mMatchMakerLoaded:" + Std.string(mMatchMakerLoaded) + " mAccountLoaded:"
			+ Std.string(mAccountLoaded) + " mTimeLoaded:" + Std.string(mTimeLoaded) + " mSteamInfoLoaded:" + Std.string(mSteamInfoLoaded));

...
class WeaponController {
	...
	function finishedAttack(autoAim:Bool, attackSpeedModifier:UInt) {
		if (mQueueAttack != null) {
			attack(mQueueAttack.Id, autoAim, attackSpeedModifier);
			mQueueAttack = null;
		}
	}
	...
	public function updateCooldown(gameClock:GameClock) {
		if (gameClock.gameTime - mTimeStartedAttack >= mCoolDownTime) {
			mIsInCooldown = false;
	...
	function updateHudCooldown(showStartCooldown:Bool) {
	...
	function getAttackFromComboArray():GMAttack {
		...
		if (_loc2_ == null) {
			Logger.error("Attack in attack array is null for attack index: " + mNextAttackComboIndex + " on weapon id: " + mWeapon.weaponData.Id
				+ " Array Size: " + mAttackArray.length);
		}
		incrementCombo();
		return _loc2_;
	}
	...
	public function canQueue(weaponQueue:PotentialWeaponInputQueueStruct, nextQueueAttackWindow:Float):Bool {
		if (mIsInCooldown) {
			return false;
		}
		...
		if (mWeaponDownActive) {
			if (weaponQueue.weaponController == this && !weaponQueue.down) {
				return true;
			}
		} else if (this.currentTimeline.getPercentageOfTimelinePlayed() >= nextQueueAttackWindow) {
			return true;
		}
		return false;
	}
	...
	public function onWeaponDown(autoAim:Bool = true) {
		mAutoAim = autoAim;
	}
	...
	public function onWeaponUp(autoAim:Bool = true) {
		mAutoAim = autoAim;
	}
	...
	public function queueAttack(attackName:String) {
		mQueueAttack = ASCompat.dynamicAs(mDBFacade.gameMaster.attackByConstant.itemFor(attackName), gameMasterDictionary.GMAttack);
		if (mQueueAttack == null) {

...
class WeaponView extends View {
	...
	public function new(facade:DBFacade, weaponGameObject:WeaponGameObject) {
		mDBFacade = facade;
		super(facade);
	...
	function applyColorAndGlow(node:DisplayObject) {
		...
		if (mWeaponGameObject.gmRarity.HasGlow) {
			node.filters = cast([
				new GlowFilter(mWeaponGameObject.gmRarity.GlowColor, 1, mWeaponGameObject.gmRarity.GlowDist, mWeaponGameObject.gmRarity.GlowDist,
					mWeaponGameObject.gmRarity.GlowStr)
			]);
		}
	}

	function removeColorAndGlow(node:DisplayObject) {
		var _loc2_ = mWeaponGameObject.weaponAesthetic;
		if (_loc2_.HasColor) {
			node.transform.colorTransform = new ColorTransform();
		}
		if (_loc2_.HasGlow) {
			node.filters = cast([]);
		}

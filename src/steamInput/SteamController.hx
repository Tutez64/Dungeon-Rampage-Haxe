...
class SteamController {
	...
	public function new(inControllerHandle:String, inGameControlsActionSet:SteamInputActionSet, inMenuControlsActionSet:SteamInputActionSet) {
		controllerHandle = inControllerHandle;
		mInGameControlsActionSet = inGameControlsActionSet;
		mMenuControlsActionSet = inMenuControlsActionSet;
		mIsConnected = false;
	}
	...
	public function set_isConnected(isConnected:Bool):Bool {
	...
	public function readInput(mSteamworks:FRESteamWorks) {
		readAnalogActions(mSteamworks);
		readDigitalActions(mSteamworks);
	}

	function clearInputDictionary(inputDictionary:ASDictionary<ASAny, ASAny>) {
		var _loc2_:ASObject;
		if (checkNullIteratee(inputDictionary))
			for (_tmp_ in inputDictionary.keys()) {
				_loc2_ = _tmp_;
				inputDictionary.remove(_loc2_);
			}
	}

	function readAnalogActions(mSteamworks:FRESteamWorks) {
		...
		if (checkNullIteratee(_loc2_))
			for (_tmp_ in _loc2_) {
				_loc3_ = _tmp_;
				_loc4_ = mSteamworks.getAnalogActionData(controllerHandle, _loc3_.actionHandle);
				mAnalogActionsNamesToVector3D[_loc3_.actionName] = new Vector3D(_loc4_.x, _loc4_.y, 0);
			}
	}

	function readDigitalActions(mSteamworks:FRESteamWorks) {
		...
		if (checkNullIteratee(_loc3_))
			for (_tmp_ in _loc3_) {
				_loc4_ = _tmp_;
				_loc5_ = mSteamworks.getDigitalActionData(controllerHandle, _loc4_.actionHandle);
				writeDigitalActionState(_loc4_.actionName, _loc5_.bState);
			}
	}

	function writeDigitalActionState(actionName:String, isDown:Bool) {
		if (isDown) {
			if (!mHeldDigitalButtons.exists(actionName)) {
				mPressedDigitalButtons[actionName] = true;
				mHeldDigitalButtons[actionName] = true;
			}
		} else if (mHeldDigitalButtons.exists(actionName)) {
			mReleasedDigitalButtons[actionName] = true;
			mHeldDigitalButtons.remove(actionName);
		}
	}
}

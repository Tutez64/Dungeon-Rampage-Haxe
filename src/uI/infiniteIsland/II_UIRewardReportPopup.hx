...
class II_UIRewardReportPopup extends DBUIPopup {
	...
	public function new(dbFacade:DBFacade, rewardData:Vector<InfiniteRewardData>, avStartScore:Int, totalGold:Int, floorGold:Int, currentFloorNum:Int,
			timeToShow:Float = 3, titleText:String = "", content:ASAny = null, allowClose:Bool = true, wantCurtain:Bool = true,
			closeCallback:ASFunction = null, useOriginalPopUp:Bool = false) {
		mFloorGold = floorGold;
		mTotalGold = totalGold;
		mCurrentFloorNum = currentFloorNum;
		mAvStartScore = avStartScore;
		mTimeRemaining = timeToShow;
		if (floorGold == 0 && mCurrentFloorNum != 0) {
			mNextGoldFloor = avStartScore + 1;
		}
		...
		if (checkNullIteratee(__ax4_iter_32))
			for (_tmp_ in __ax4_iter_32) {
				...
				if (_loc14_.status == 0) {
					mNextChestFloor = (_loc14_.floorNumber : Int);
					break;
				}
			}
		super(dbFacade, titleText, content, allowClose, wantCurtain, closeCallback, useOriginalPopUp);
	...
	function tickCountdown(event:TimerEvent) {
		ASCompat.setProperty((mPopup : ASAny).countdown_bounce.countdown.countdown_text, "text", "" + Std.int(mTimeRemaining));
		if (mTimeRemaining <= 0) {
			endCountdown();
			destroy();

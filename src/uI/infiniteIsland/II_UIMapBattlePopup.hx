...
class II_UIMapBattlePopup extends UIMapBattlePopup {
	...
	public function new(dbFacade:DBFacade, changeHeroReleaseCallback:ASFunction, changeStatReleaseCallback:ASFunction, changeShopReleaseCallback:ASFunction,
			battleReleaseCallback:ASFunction, titleText:String) {
		super(dbFacade, changeHeroReleaseCallback, changeStatReleaseCallback, changeShopReleaseCallback, battleReleaseCallback, titleText);
	...
	override function setupUI(swfAsset:SwfAsset, titleText:String, content:ASAny, allowClose:Bool, closeCallback:ASFunction) {
		...
		i = 0;
		while (i < 4) {
			mFloorModifierLabels.push(new UIObject(mDBFacade,
				ASCompat.reinterpretAs(mPopup.getChildByName("floor_modifier_" + Std.string((i + 1))), MovieClip)));
			i = i + 1;
		}
		...
		i = 0;
		if (checkNullIteratee(friendsKeyArray))
			for (_tmp_ in friendsKeyArray) {
	...
	override public function setDungeonDetails() {
		...
		while (_loc3_ < _loc2_.modifiers.length) {
			ASCompat.setProperty((mFloorModifierLabels[_loc3_].root : ASAny).label_floor_modifier, "text",
				Locale.getString("BATTLE_POP_UP_II_FLOOR:") + Std.string(_loc5_.DModFloorStart[_loc3_]));
			_loc3_++;
		}
		_loc3_ = 0;
		while (_loc3_ < _loc2_.modifiers.length) {
	...
	function itemsLoaded(swfAsset:SwfAsset) {
		mItemAsset = swfAsset;
		if (mReadyForChests) {
			populateChests();
		}
	...
	function onReceivedTopScores(result:ASAny) {
		if (result == false) {
			return;
		}
		mTopTwentyScores.add(mCurrentDungeon.Id, new II_ChampionsboardListPerNode(result));
	}
	...
	function switchChampionsBoardType() {
		...
		if (mIsChampionsBoardFriends) {
			...
			while (_loc3_ < mChampionsboardFriendSlots.length) {
				mChampionsboardFriendSlots[_loc3_].setDungeonDetails((mCurrentDungeon.Id : Int), mFriends[_loc3_],
					mFriends[_loc3_].id == mDBFacade.dbAccountInfo.id);
	...
	function sortFriends(f1:FriendInfo, f2:FriendInfo):Int {
		return f2.getIILeaderboardScoreForNode((mCurrentDungeon.Id : Int)) - f1.getIILeaderboardScoreForNode((mCurrentDungeon.Id : Int));
	}

	function lateLoadForModifier(index:UInt, gmDungeonModifer:GMDungeonModifier):ASFunction {
		return function(param1:SwfAsset) {
			...
			ASCompat.setProperty((mFloorModifierLabels[(index : Int)].tooltip : ASAny).title_label, "text",
				GameMasterLocale.getGameMasterSubString("DUNGEON_MODIFIER_NAME", gmDungeonModifer.Constant));
			ASCompat.setProperty((mFloorModifierLabels[(index : Int)].tooltip : ASAny).description_label, "text",
				GameMasterLocale.getGameMasterSubString("DUNGEON_MODIFIER_DESCRIPTION", gmDungeonModifer.Constant));
		};
	}

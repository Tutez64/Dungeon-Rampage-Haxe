...
class UILevelUpShopPopup extends DBUIOneButtonPopup {
	...
	public function new(dbFacade:DBFacade, centerCallback:ASFunction, avatarInfo:AvatarInfo, gmOffers:Vector<GMOffer>) {
		mAvatarInfo = avatarInfo;
		mGMOffers = gmOffers;
		super(dbFacade, Locale.getString("LEVELUPSHOP_POPUP_TITLE"), Locale.getString("LEVELUPSHOP_POPUP_MESSAGE"),
			Locale.getString("LEVELUPSHOP_POPUP_BUTTON"), centerCallback, true, null);
		mDBFacade.metrics.log("LevelUpShopPopupPresented");
	}

	public static function getLevelUpWeaponUnlocks(dbFacade:DBFacade, avatarInfo:AvatarInfo):Vector<GMOffer> {
		...
		var _loc3_ = avatarInfo.level;
		var _loc5_ = dbFacade.gameMaster.Offers;
		_loc9_ = 0;
				...
				if (!_loc10_.Gift) {
					if (!_loc10_.IsBundle) {
						if (_loc10_.Details[0].WeaponId != 0) {
							_loc11_ = (StoreServicesController.requiredHeroForWeapon(dbFacade, _loc10_) : Int);
							if (!(_loc11_ != 0 && (_loc11_ : UInt) != avatarInfo.avatarType)) {
								_loc8_ = (StoreServicesController.getOfferLevelReq(dbFacade, _loc10_) : Int);
								if (!(_loc8_ != 0 && (_loc8_ : UInt) != _loc3_)) {
									_loc6_ = StoreServicesController.getWeaponMastertype(dbFacade, _loc10_);
									if (!(ASCompat.stringAsBool(_loc6_) && !avatarInfo.gmHero.AllowedWeapons.hasKey(_loc6_))) {
										_loc4_.push(_loc10_);
									}
								}
							}
	...
	override function setupUI(swfAsset:SwfAsset, titleText:String, content:ASAny, allowClose:Bool, closeCallback:ASFunction) {
		...
		if (checkNullIteratee(offerUIs))
			for (_tmp_ in offerUIs) {
				offerUI = _tmp_;
	...
	function loadWeaponIcon(parent:DisplayObjectContainer, gmWeaponItem:GMWeaponItem) {

...
class UINewsFeedController {
	...
	function assetLoaded(swfAsset:SwfAsset) {
		mNewsFeedClass = swfAsset.getClass("UI_alertPrompt");
	}

	function friendOnlineStatusUpdate(event:FriendStatusEvent) {
		var _loc3_ = mDBFacade.gameObjectManager.getReferenceFromId(event.friendId);
		var _loc4_ = ASCompat.reinterpretAs(_loc3_, PlayerGameObject);
		if (_loc4_ != null) {
			return;
		}
		var _loc2_ = new UINewsFeed(mDBFacade, ASCompat.dynamicAs(ASCompat.createInstance(mNewsFeedClass, []), flash.display.MovieClip), removeNewsFeed,
			(0 : UInt), event);
		if (_loc2_.isValid) {
	...
	function friendSummaryStatusUpdate(event:FriendSummaryNewsFeedEvent) {
		var _loc2_ = new UINewsFeed(mDBFacade, ASCompat.dynamicAs(ASCompat.createInstance(mNewsFeedClass, []), flash.display.MovieClip), removeNewsFeed,
			(3 : UInt), event);
		if (_loc2_.isValid) {
			mQueuedFeeds.push(_loc2_);
	...
	function friendDungeonStatusUpdate(event:FriendStatusEvent) {
		...
		if (!ASCompat.toBool(mDBFacade.dbAccountInfo.friendInfos.itemFor(event.friendId))) {
			return;
		}
		var _loc2_ = new UINewsFeed(mDBFacade, ASCompat.dynamicAs(ASCompat.createInstance(mNewsFeedClass, []), flash.display.MovieClip), removeNewsFeed,
			(1 : UInt), event);
	...
	function genericNewsFeedMessage(event:GenericNewsFeedEvent) {
		var _loc2_ = new UINewsFeed(mDBFacade, ASCompat.dynamicAs(ASCompat.createInstance(mNewsFeedClass, []), flash.display.MovieClip), removeNewsFeed,
			(2 : UInt), event);
		if (_loc2_.isValid) {
			mQueuedFeeds.push(_loc2_);
	...
	function checkNewsFeedQueue(gameClock:GameClock) {
	...
	function removeNewsFeed(gameClock:GameClock) {

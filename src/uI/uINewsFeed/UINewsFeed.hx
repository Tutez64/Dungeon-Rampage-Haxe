...
class UINewsFeed {
	...
	public function new(dbFacade:DBFacade, root:MovieClip, finishedCallback:ASFunction, feedType:UInt, event:Event = null) {
		mDBFacade = dbFacade;
		mRoot = root;
		...
		setupUI();
		if (Std.isOfType(event, FriendStatusEvent)) {
			mFriendEvent = ASCompat.reinterpretAs(event, FriendStatusEvent);
			loadFriendDetails();
		} else if (Std.isOfType(event, GenericNewsFeedEvent)) {
			mGenericFeedMessageEvent = ASCompat.reinterpretAs(event, GenericNewsFeedEvent);
		} else if (Std.isOfType(event, FriendSummaryNewsFeedEvent)) {
			mFriendSummaryFeedMessageEvent = ASCompat.reinterpretAs(event, FriendSummaryNewsFeedEvent);
		} else {
			mIsValid = false;
		}
		switch (feedType) {
	...
	public function lerpOutOfScreen(gameClock:GameClock) {
	...
	public function colorizeMessage(tf:TextFormat, start:Int, end:Int) {
		mLabel.setTextFormat(tf, start, end);
	}
	...
	function prepareFriendSummaryMessageFeed() {
		...
		if (mFriendSummaryFeedMessageEvent.isFriendNameInFront) {
			...
			colorizeMessage(_loc1_, mFriendSummaryFeedMessageEvent.message.length,
				mFriendSummaryFeedMessageEvent.message.length + mFriendSummaryFeedMessageEvent.friendName.length);
		}
		adjustLabelHeight();

...
class UITavern {
	...
	public function new(dbFacade:DBFacade, root:MovieClip, townHeader:TownHeader) {
		...
		mLogicalWorkComponent = new LogicalWorkComponent(dbFacade, "UITavern");
		mRootMovieClip = root;
	...
	function loadTavernInfo() {
		...
		mWeaponIcons3.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_3.weapon_icon_slot_0, flash.display.MovieClip)));
		mWeaponIcons3.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_3.weapon_icon_slot_1, flash.display.MovieClip)));
		mWeaponIcons3.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_3.weapon_icon_slot_2, flash.display.MovieClip)));
		mWeaponIcons4.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_4.weapon_icon_slot_0, flash.display.MovieClip)));
		mWeaponIcons4.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_4.weapon_icon_slot_1, flash.display.MovieClip)));
		mWeaponIcons4.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_4.weapon_icon_slot_2, flash.display.MovieClip)));
		mWeaponIcons4.push(new UIObject(mDBFacade,
			ASCompat.dynamicAs((mTavernInfoClip : ASAny).character_info.weapons_4.weapon_icon_slot_3, flash.display.MovieClip)));
		...
		ASCompat.ASArray.pushMultiple(mAttackStars, ASCompat.dynamicAs((mStoryInfoClip : ASAny).attack_star_0, flash.display.MovieClip),
			(mStoryInfoClip : ASAny).attack_star_1, (mStoryInfoClip : ASAny).attack_star_2, (mStoryInfoClip : ASAny).attack_star_3,
			(mStoryInfoClip : ASAny).attack_star_4);
		mDefenseStars = new Vector<MovieClip>();
		ASCompat.ASArray.pushMultiple(mDefenseStars, ASCompat.dynamicAs((mStoryInfoClip : ASAny).defense_star_0, flash.display.MovieClip),
			(mStoryInfoClip : ASAny).defense_star_1, (mStoryInfoClip : ASAny).defense_star_2, (mStoryInfoClip : ASAny).defense_star_3,
			(mStoryInfoClip : ASAny).defense_star_4);
		mSpeedStars = new Vector<MovieClip>();
		ASCompat.ASArray.pushMultiple(mSpeedStars, ASCompat.dynamicAs((mStoryInfoClip : ASAny).speed_star_0, flash.display.MovieClip),
			(mStoryInfoClip : ASAny).speed_star_1, (mStoryInfoClip : ASAny).speed_star_2, (mStoryInfoClip : ASAny).speed_star_3,
			(mStoryInfoClip : ASAny).speed_star_4);
		...
		mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath("Resources/Art2D/UI/db_UI_town.swf"),
	...
	function getSkinsToDisplay(gmHero:GMHero):Vector<GMSkin> {
		return mDBFacade.dbAccountInfo.inventoryInfo.getSkinsForHero(gmHero, mSkinOffers);
	}

	function heroIdSort(id1:UInt, id2:UInt):Int {
		if (id1 < id2) {
			return -1;
		}
		return 1;
	...
	public function refresh() {
		...
		setupHeaderLinks();
		setupHeroSlotsNavigationLinks();
		setupUsableWeaponsMenuNavigations();
		_loc2_ = 0;
	...
	function loadAvatarHolderAtPosition(heroIndex:Int, vectorIndex:Int) {
		var gmSkin:GMSkin;
		var iconName:String;
	...
	function scrollSkins(scrollAmount:Int) {
		...
		var _loc4_ = mSkinStartIndex + scrollAmount;
	...
	function handleScaledSlotIndex(count:Int) {
		mScaledSlotIndex += count;
	...
	function populateAvatarInfo(index:Int) {
		resizeHeroSlots((index : UInt));
		index += mAvatarSelectorStartIndex;
		if (index >= mHeroIds.length) {
			return;
		}
		var _loc3_ = ASCompat.dynamicAs(mDBFacade.gameMaster.heroById.itemFor(mHeroIds[index]), gameMasterDictionary.GMHero);
		chooseThisHero(_loc3_.Id, index - mAvatarSelectorStartIndex);
		ASCompat.setProperty((mTavernInfoClip : ASAny).avatar_name_text, "text",
			GameMasterLocale.getGameMasterSubString("SKIN_NAME", _loc3_.Constant).toUpperCase());
		var _loc2_ = mDBFacade.dbAccountInfo.inventoryInfo.getAvatarInfoForHeroType(_loc3_.Id);
		if (_loc2_ != null) {
			setXP(_loc2_.experience, _loc3_);
			mDBFacade.steamAchievementsManager.setHeroLevelStat(_loc2_.level, _loc2_.gmHero);
		}
		mXPParentObject.visible = _loc2_ != null;
		populateWeaponIcons(index);
	...
	function populateHeroStars(gmHero:GMHero) {
		var _loc2_ = 0;
		_loc2_ = 0;
		while (_loc2_ < mAttackStars.length) {
			mAttackStars[_loc2_].visible = gmHero.AttackRating - 1 >= _loc2_;
			mDefenseStars[_loc2_].visible = gmHero.DefenseRating - 1 >= _loc2_;
			mSpeedStars[_loc2_].visible = gmHero.SpeedRating - 1 >= _loc2_;
			_loc2_ = ASCompat.toInt(_loc2_) + 1;
		}
	}

	function setupSkin(gmSkin:GMSkin) {
		...
		mSkinNameLabel.text = GameMasterLocale.getGameMasterSubString("SKIN_NAME", gmSkin.Constant).toUpperCase();
		setUpSkinSelector();
	}

	function setupBuyButtonWithSkin(gmSkin:GMSkin, ownsHero:Bool) {
		var nowTime:Float;
		var offer:GMOffer;
		...
		ASCompat.setProperty((mBuyButton.root : ASAny).dlc_cash_icon, "visible", currentOffer.CurrencyType == "DLC");
		mBuyButton.releaseCallback = function() {
			buySkin(gmSkin.Id);
		};
		if (currentOffer.CurrencyType == "DLC") {
			mBuyButton.visible = true;
			mBuyButtonCoin.visible = false;
			mBuyButtonCash.visible = false;
			ASCompat.setProperty((mSkinInfoClip : ASAny).label_or, "visible", false);
			mRecruitButton.visible = false;
			ASCompat.setProperty((mBuyButton.root : ASAny).cost_text, "visible", false);
			ASCompat.setProperty((mBuyButton.root : ASAny).cash_icon, "visible", currentOffer.CurrencyType == "PREMIUM");
			ASCompat.setProperty((mBuyButton.root : ASAny).coin_icon, "visible", currentOffer.CurrencyType == "BASIC");
			ASCompat.setProperty((mBuyButton.root : ASAny).dlc_cash_icon, "visible", currentOffer.CurrencyType == "DLC");
			mBuyButton.releaseCallback = function() {
				mDBFacade.overlaySteamBasedOnAppId(offer.SteamAppId);
			};
		}
		if (ownsHero) {
			mBuyButton.enabled = true;
		...
		if (offer != null
			&& offer.SaleStartDate != null
			&& offer.SaleEndDate != null
			&& offer.SaleStartDate.getTime() <= nowTime
			&& offer.SaleEndDate.getTime() > nowTime) {
			mBuyButton.label.text = Locale.getString("TAVERN_SKIN_ON_SALE");
		}
	}

	function setupBuyButtonForHero(gmHero:GMHero) {
		var offer:GMOffer;
		var saleOffer:GMOffer;
	...
	function determineSkinForHero(gmHero:GMHero):GMSkin {
		var _loc3_:GMSkin = null;
		var _loc2_ = mDBFacade.dbAccountInfo.inventoryInfo.getAvatarInfoForHeroType(gmHero.Id);
		if (_loc2_ != null) {
			if (mHeroSelectionsToFlush.hasKey(gmHero.Id)) {
				_loc3_ = mDBFacade.gameMaster.getSkinByType((ASCompat.toInt(mHeroSelectionsToFlush.itemFor(gmHero.Id)) : UInt));
			} else {
		...
		if (_loc3_ == null) {
			_loc3_ = mDBFacade.dbAccountInfo.inventoryInfo.getDefaultSkinForHero(gmHero);
		}
		return _loc3_;
	}

	function determineSkinSelectorStartIndex(gmSkin:GMSkin) {
		var _loc4_ = 0;
		var _loc3_ = ASCompat.dynamicAs(mDBFacade.gameMaster.heroById.itemFor(mHeroIds[mCurrentChosenIndex]), gameMasterDictionary.GMHero);
		var _loc2_ = this.getSkinsToDisplay(_loc3_);
		_loc4_ = 0;
		while (_loc4_ < _loc2_.length) {
			if (_loc2_[_loc4_].Id == gmSkin.Id) {
	...
	function skinSelected(gmSkin:GMSkin) {
		mSelectedSkin = gmSkin;
		if (gmSkin != null) {
			ASCompat.setProperty((mStoryInfoClip : ASAny).avatar_description_text, "text",
				GameMasterLocale.getGameMasterSubString("SKIN_DESCRIPTION", gmSkin.Constant));
			ASCompat.setProperty((mStoryInfoClip : ASAny).avatar_likes_text, "text",
				GameMasterLocale.getGameMasterSubString("SKIN_LIKES", gmSkin.Constant).toUpperCase());
			ASCompat.setProperty((mStoryInfoClip : ASAny).avatar_dislikes_text, "text",
				GameMasterLocale.getGameMasterSubString("SKIN_DISLIKES", gmSkin.Constant).toUpperCase());
			ASCompat.setProperty((mStoryInfoClip : ASAny).avatar_nickname_text, "text",
				GameMasterLocale.getGameMasterSubString("SKIN_NICKNAME", gmSkin.Constant).toUpperCase());
		}
		var _loc4_ = mDBFacade.dbAccountInfo.inventoryInfo.doesPlayerOwnSkin(gmSkin.Id);
		...
		if (_loc2_) {
			if (_loc4_) {
				if (mHeroSelectionsToFlush.hasKey(_loc3_.gmHero.Id)) {
					mHeroSelectionsToFlush.replaceFor(_loc3_.gmHero.Id, gmSkin.Id);
				} else {
					mHeroSelectionsToFlush.add(_loc3_.gmHero.Id, gmSkin.Id);
				}
				mChosenSkin = gmSkin;
			}
		} else {
	...
	function setHeroAndSkin() {
		...
		while (ASCompat.toBool(_loc1_.next())) {
			_loc3_ = ASCompat.toInt(_loc1_.key);
			_loc4_ = (ASCompat.asUint(_loc1_.current) : Int);
			setSkinOnAvatar((_loc3_ : UInt), (_loc4_ : UInt));
		}
		mHeroSelectionsToFlush.clear();
	}

	function setSkinOnAvatar(heroId:UInt, skinId:UInt) {
		var _loc4_ = false;
		var _loc3_ = mDBFacade.dbAccountInfo.inventoryInfo.getAvatarInfoForHeroType(heroId);
		if (_loc3_ != null) {
			_loc4_ = mDBFacade.dbAccountInfo.inventoryInfo.doesPlayerOwnSkin(skinId);
			if (!_loc4_) {
				Logger.error("Trying to set skin as active that the user does no own.");
				return;
			}
			if (_loc3_.skinId != skinId) {
				_loc3_.skinId = skinId;
				if (mActiveAvatar.id == _loc3_.id) {
					mActiveAvatar.skinId = skinId;
				}
	...
	function resizeHeroSlots(selectedIndex:UInt) {
		var _loc2_ = 0;
		mScaledSlotIndex = (selectedIndex : Int);
		_loc2_ = 0;
		while (_loc2_ < mHeroSlots.length) {
			if ((_loc2_ : UInt) == selectedIndex) {
	...
	public function select(root:MovieClip) {
		root.scaleX = 1.25;
		root.scaleY = 1.25;
	}

	public function deselect(root:MovieClip) {
		root.scaleX = 1;
		root.scaleY = 1;
	}
	...
	public function populateWeaponIcons(index:Int) {
		...
		var weaponIconCallback:ASFunction = function(param1:Int, param2:GMWeaponMastertype):ASFunction {
			...
			return function(param1:brain.assetRepository.SwfAsset) {
				...
				var _loc2_ = ASCompat.dynamicAs(cast((mWeaponIconsVector[index].root : ASAny).bg_slot, flash.display.DisplayObjectContainer)
				...
				ASCompat.setProperty((mWeaponIconsVector[index].tooltip : ASAny).title_label, "text",
					GameMasterLocale.getGameMasterSubString("WEAPON_MASTERTYPE", subType.Constant).toUpperCase());
			};
		};
		...
		if (checkNullIteratee(allowedWeaponSubTypes))
			for (_tmp_ in allowedWeaponSubTypes) {
				masterTypeString = _tmp_;
				masterType = ASCompat.dynamicAs(mDBFacade.gameMaster.weaponSubtypeByConstant.itemFor(masterTypeString),
					gameMasterDictionary.GMWeaponMastertype);
				if (masterType != null) {
		...
		if (masterTypeVector.length == 3) {
			...
			mScrollSkinsRightButton.clearRightNavigation();
			mWeaponIcons3[0].clearLeftNavigation();
			mScrollSkinsRightButton.isToTheLeftOf(mWeaponIcons3[0]);
			...
			mScrollSkinsRightButton.clearRightNavigation();
			mWeaponIcons4[0].clearLeftNavigation();
			mScrollSkinsRightButton.isToTheLeftOf(mWeaponIcons4[0]);
		} else {
		...
		if (checkNullIteratee(masterTypeVector))
			for (_tmp_ in masterTypeVector) {
				masterType = _tmp_;
				mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath(masterType.UISwfFilepath),
					ASCompat.asFunction(weaponIconCallback(weaponIconIndex, masterType)));
	...
	function chooseThisHero(avatarID:UInt, index:Int) {
		var _loc4_ = 0;
		var _loc3_ = mDBFacade.dbAccountInfo.inventoryInfo.getAvatarInfoForHeroType(avatarID);
		if (_loc3_ != null) {
			...
			if (ASCompat.toBool((mHeroSlots[index].root : ASAny).chosen)) {
				ASCompat.setProperty((mHeroSlots[index].root : ASAny).chosen, "visible", true);
			}
		}
		mCurrentChosenIndex = index;
		mChosenAvatarID = avatarID;
	}

	function buyThisHero(offer:GMOffer, index:UInt) {
		StoreServicesController.tryBuyOffer(mDBFacade, offer, function(param1:ASAny) {
			var _loc2_ = offer.Details[0].HeroId;
			DBFacebookBragFeedPost.buyHeroSuccess(mTownHeader, ASCompat.dynamicAs(mDBFacade.gameMaster.heroById.itemFor(_loc2_), gameMasterDictionary.GMHero),
				mDBFacade, mAssetLoadingComponent);
			populateAvatarInfo((index : Int));
			populateAvatarSelector();
		});
	}

	function buySkin(skinId:UInt) {
	...
	public function set_avatarSelectorStartIndex(value:Int):Int {
		if (value == mAvatarSelectorStartIndex) {
			return value;
		}
		mAvatarSelectorStartIndex = value;
		populateAvatarSelector();
	...
	public function set_currentChosenIndex(value:Int):Int {
		return mCurrentChosenIndex = value;
	}
	...
	function changeActiveAvatarRPC(heroId:UInt) {
		mDBFacade.dbAccountInfo.changeActiveAvatarRPC(heroId);
	}
	...
	public function setupHeaderLinks() {
		mTownHeader.addCoinButton.isAbove(mSkinVector[0]);
		mTownHeader.addGemButton.isAbove(mSkinVector[1]);
		mTownHeader.closeButton.isAbove(mSkinVector[2]);
	}

	function setupNavigationLinks() {
		mScrollSkinsLeftButton.isToTheLeftOf(mSkinVector[0]);
		mSkinVector[0].isToTheLeftOf(mSkinVector[1]);
		mSkinVector[1].isToTheLeftOf(mSkinVector[2]);
		mSkinVector[2].isToTheLeftOf(mScrollSkinsRightButton);
		mSkinVector[0].downNavigation = mBuyButton;
		mSkinVector[1].downNavigation = mBuyButton;
		mSkinVector[2].downNavigation = mBuyButton;
		mBuyButton.upNavigation = mSkinVector[0];
		mBuyButton.downNavigation = mHeroSlots[0];
		mHeroSlots[0].upNavigation = mBuyButton;
	}

	public function setupHeroSlotsNavigationLinks() {
		...
		mHeroSlots[1].upNavigation = mBuyButton;
		mHeroSlots[2].upNavigation = mBuyButton;
		mHeroSlots[3].upNavigation = mBuyButton;
		mHeroSlots[4].upNavigation = mBuyButton;
		mHeroSlots[5].upNavigation = mBuyButton;
		mHeroSlots[6].upNavigation = mBuyButton;
		mHeroSlots[7].upNavigation = mBuyButton;
	}

	public function resetHeaderLinks() {
		mScrollSkinsLeftButton.clearNavigationAndInteractions();
		mScrollSkinsRightButton.clearNavigationAndInteractions();
		mSkinVector[0].clearNavigationAndInteractions();
		mSkinVector[1].clearNavigationAndInteractions();
		mSkinVector[2].clearNavigationAndInteractions();
		mBuyButton.clearNavigationAndInteractions();
		...
		mWeaponIcons3[0].clearNavigationAndInteractions();
		mWeaponIcons3[1].clearNavigationAndInteractions();
		mWeaponIcons3[2].clearNavigationAndInteractions();
		mWeaponIcons4[0].clearNavigationAndInteractions();
		mWeaponIcons4[1].clearNavigationAndInteractions();
		mWeaponIcons4[2].clearNavigationAndInteractions();
		mWeaponIcons4[3].clearNavigationAndInteractions();
	}

	public function setupUsableWeaponsMenuNavigations() {
		mWeaponIcons3[0].isToTheLeftOf(mWeaponIcons3[1]);
		mWeaponIcons3[1].isToTheLeftOf(mWeaponIcons3[2]);
		mWeaponIcons4[0].isToTheLeftOf(mWeaponIcons4[1]);
		mWeaponIcons4[1].isToTheLeftOf(mWeaponIcons4[2]);
		mWeaponIcons4[2].isToTheLeftOf(mWeaponIcons4[3]);
	}

	public function getDefaultMenuNavigationObject():UIObject {
		return mSkinVector[0];
	}
}

...
class UITavernSkinButton extends UIRadioButton {
	...
	public function new(dbFacade:DBFacade, root:MovieClip, showSkinInTavernCallback:ASFunction) {
		mDBFacade = dbFacade;
		super(dbFacade, root, "skin_selector_buttons");
		...
		mShowSkinInTavernCallback = showSkinInTavernCallback;
	...
	public function set_chosen(val:Bool):Bool {
		return mChosenClip.visible = val;
	}

	@:isVar public var gmSkin(never, set):GMSkin;

	public function set_gmSkin(val:GMSkin):GMSkin {
		mGMSkin = val;
		refresh();
		return val;
	}

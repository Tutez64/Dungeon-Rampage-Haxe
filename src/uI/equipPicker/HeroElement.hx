...
class HeroElement extends UIButton {
	...
	public function new(dbFacade:DBFacade, root:MovieClip, tooltipClass:Dynamic, heroClicked:ASFunction) {
		super(dbFacade, root);
		mDBFacade = dbFacade;
	...
	public function showWeaponComparison(gmWeaponItem:GMWeaponItem, power:UInt) {
		...
		var _loc5_ = mGMHero.AllowedWeapons.hasKey(gmWeaponItem.MasterType);
		if (_loc5_) {
			...
			if (_loc3_ != null) {
				...
				if (checkNullIteratee(_loc6_))
					for (_tmp_ in _loc6_) {
						_loc7_ = _tmp_;
						if (_loc7_.gmWeaponItem.MasterType == gmWeaponItem.MasterType && _loc7_.power >= power) {
							_loc4_ = false;
							break;
						}
					}
	...
	public function set_gmHero(hero:GMHero):GMHero {

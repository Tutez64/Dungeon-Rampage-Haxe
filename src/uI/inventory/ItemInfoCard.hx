...
class ItemInfoCard extends UIObject {
	...
	public function new(dbFacade:DBFacade, root:MovieClip, chargeTooltipClass:Dynamic, heroEquipPicker:HeroWithEquipPicker,
			petEquipPicker:PetsWithEquipPicker, refreshInventoryCallback:ASFunction, dungeonSummary:DistributedDungeonSummary, allowCloseButton:Bool = true,
			sellItemCallback:ASFunction = null, takeItemCallback:ASFunction = null, closeButtonCallback:ASFunction = null) {
		super(dbFacade, root);
		mDBFacade = dbFacade;
	...
	public function enableCloseButton(callback:ASFunction) {
		mCloseButton.visible = true;
		mCloseButton.releaseCallback = callback;
	}
	...
	public function set_info(info:InventoryBaseInfo):InventoryBaseInfo {
	...
	function setupInventoryBaseUI(type:String) {
	...
	function setupStackableInfoUI(equipped:Bool) {
		...
		var _loc3_ = (1 : UInt);
		mEquipButton.enabled = equipped ? true : _loc3_ == 1;
	...
	function setupWeaponItemUI(itemInfo:ItemInfo, equipped:Bool) {
			...
			if (ASCompat.stringAsBool(swfPath) && ASCompat.stringAsBool(iconName)) {
				mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath(swfPath), function(param1:brain.assetRepository.SwfAsset) {
					...
					if (_loc3_ != null) {
						...
						mTapTooltip.setValues(GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_TAP_TITLE", gmWeaponItem.Constant),
							GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_TAP_DESCRIPTION", gmWeaponItem.Constant));
			...
			if (ASCompat.stringAsBool(swfPath1) && ASCompat.stringAsBool(iconName1)) {
				mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath(swfPath1), function(param1:brain.assetRepository.SwfAsset) {
					...
					if (_loc3_ != null) {
						...
						_loc4_ = gmWeaponItem.WeaponController != null ? GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_CONTROLLER",
							gmWeaponItem.WeaponController) : gmWeaponItem.HoldTitle;
		...
		if (currentlySelectedAvatarInfo != null
			&& mDBFacade.dbAccountInfo.inventoryInfo.canAvatarEquipThisMasterType(currentlySelectedAvatarInfo, gmWeaponItem.MasterType)) {
			mWeaponTypeUnequippableLabel.visible = false;
		...
		while (i < modifierList.length) {
			mModifiersList.push(new UIModifier(mDBFacade,
				ASCompat.dynamicAs(cast((mRoot : ASAny).ability, flash.display.DisplayObjectContainer).getChildByName("modifier_icon_" + Std.string((i + 1))),
					MovieClip),
				modifierList[i].Constant));
		...
		if (itemInfo.legendaryModifier > 0) {
			mModifiersList.push(new UIModifier(mDBFacade,
				ASCompat.dynamicAs(cast((mRoot : ASAny).ability, flash.display.DisplayObjectContainer).getChildByName("modifier_icon_3"), MovieClip), "",
				(0 : UInt), true, itemInfo.legendaryModifier));
	...
	function greyOffer(value:Bool) {
		var _loc4_ = Vector.ofArray(([
			ASCompat.dynamicAs((mRoot : ASAny).bg, flash.display.DisplayObject),
			ASCompat.dynamicAs((mRoot : ASAny).icon_slot, flash.display.DisplayObject),
			ASCompat.dynamicAs((mRoot : ASAny).ribbon, flash.display.DisplayObject),
			mHoldIcon.root,
			mTapIcon.root,
			mPowerIcon,
			mItemParent.root,
			ASCompat.dynamicAs((mRoot : ASAny).unequippable, flash.display.DisplayObject),
			ASCompat.dynamicAs((mRoot : ASAny).ability.modifier_icon_1, flash.display.DisplayObject),
			ASCompat.dynamicAs((mRoot : ASAny).ability.modifier_icon_2, flash.display.DisplayObject),
			ASCompat.dynamicAs((mRoot : ASAny).ability.modifier_icon_3, flash.display.DisplayObject)
		] : Array<DisplayObject>));
		...
		if (checkNullIteratee(_loc4_))
			for (_tmp_ in _loc4_) {
				_loc3_ = _tmp_;
				if (_loc3_ != null) {
					_loc3_.filters = cast(value ? [_loc2_] : []);
				}
			}
		mLabel.textColor = (ASCompat.toInt(value ? (8947848 : UInt) : mInfo.getTextColor()) : UInt);
	}

	public function buttonsVisible(makeVisible:Bool) {
		mEquipButton.visible = makeVisible;
		mSellButton.visible = makeVisible;
	}

	function setupWeaponItemInfoUI(itemInfo:ItemInfo) {
	...
	function unEquipConsumable(itemInfo:StackableInfo, successCallback:ASFunction) {
		...
		mDBFacade.dbAccountInfo.inventoryInfo.unequipConsumableOffAvatar(_loc3_.id, itemInfo.gmStackable.Id, (itemInfo.equipSlot : UInt), successCallback);
	}

	function setupStackableItemInfoUI(itemInfo:StackableInfo) {
		mEquipButton.enabled = false;
		mEquipButton.visible = false;
	...
	function loadIcon() {
		...
		ItemInfo.loadItemIcon(swfPath, iconName, mItemParent.root, mDBFacade, (70 : UInt), (Std.int(mInfo.iconScale) : UInt), mAssetLoadingComponent,
			function() {
	...
	function loadItemType(gmWeaponItem:GMWeaponItem) {
		var _loc3_:String = null;
		var _loc2_:GMWeaponMastertype = null;
		if (gmWeaponItem != null) {
			_loc3_ = gmWeaponItem.MasterType;
			...
			if (_loc2_ != null) {
				mWeaponTypeLabel.visible = true;
				mWeaponTypeUnequippableLabel.visible = true;
				mWeaponTypeLabel.text = GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_NAME", gmWeaponItem.Constant).toUpperCase();
				mWeaponTypeUnequippableLabel.text = GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_NAME", gmWeaponItem.Constant).toUpperCase();
			} else {
				mWeaponTypeLabel.visible = false;
				mWeaponTypeUnequippableLabel.visible = false;
	...
	public function loadEquipOnAvatarPopup(isConsumable:Bool = false) {
		if (mEquipPopup != null) {
			mEquipPopup.destroy();
		}
		...
		mEquipPopup = new EquipItemToSlotPopup(mDBFacade, (_loc4_.id : Int), closeEquipCallback,
			ASCompat.dynamicAs(isConsumable ? _loc3_ : _loc2_, account.InventoryBaseInfo), null, isConsumable);

...
class EquipItemToSlotPopup extends DBUIPopup {
	...
	public function new(dbFacade:DBFacade, avatarInstanceId:Int, closePopupCallback:ASFunction, itemInfoToEquip:InventoryBaseInfo,
			notAllowedCallback:ASFunction = null, isConsumable:Bool = false) {
		mDBFacade = dbFacade;
		mAvatarInstanceId = avatarInstanceId;
		mClosePopupCallback = closePopupCallback;
		mNotAllowedCallback = notAllowedCallback;
		mItemInfoToEquip = itemInfoToEquip;
	...
	override function setupUI(swfAsset:SwfAsset, titleText:String, content:ASAny, allowClose:Bool, closeCallback:ASFunction) {
		mSwfAsset = swfAsset;
		var _loc7_ = swfAsset.getClass("popup_equip");
	...
	function refreshAvatarConsumableSlots() {
		...
		if (equippedItemInfo0 != null) {
			ItemInfo.loadItemIconFromId(equippedItemInfo0.gmId, ASCompat.dynamicAs((mEquipSlot0.root : ASAny).graphic, flash.display.DisplayObjectContainer),
				mDBFacade, (70 : UInt), (Std.int(equippedItemInfo0.iconScale) : UInt), mAssetLoadingComponent);
		...
		if (equippedItemInfo1 != null) {
			ItemInfo.loadItemIconFromId(equippedItemInfo1.gmId, ASCompat.dynamicAs((mEquipSlot1.root : ASAny).graphic, flash.display.DisplayObjectContainer),
				mDBFacade, (70 : UInt), (Std.int(equippedItemInfo1.iconScale) : UInt), mAssetLoadingComponent);
	...
	function refreshAvatarWeaponSlots() {
		...
		if (equippedItemInfo0 != null) {
			ItemInfo.loadItemIconFromItemInfo(equippedItemInfo0, ASCompat.dynamicAs((mEquipSlot0.root : ASAny).graphic, flash.display.DisplayObjectContainer),
				mDBFacade, (70 : UInt), (Std.int(equippedItemInfo0.iconScale) : UInt), mAssetLoadingComponent);
		...
		if (equippedItemInfo1 != null) {
			ItemInfo.loadItemIconFromItemInfo(equippedItemInfo1, ASCompat.dynamicAs((mEquipSlot1.root : ASAny).graphic, flash.display.DisplayObjectContainer),
				mDBFacade, (70 : UInt), (Std.int(equippedItemInfo1.iconScale) : UInt), mAssetLoadingComponent);
		...
		if (equippedItemInfo2 != null) {
			ItemInfo.loadItemIconFromItemInfo(equippedItemInfo2, ASCompat.dynamicAs((mEquipSlot2.root : ASAny).graphic, flash.display.DisplayObjectContainer),
				mDBFacade, (70 : UInt), (Std.int(equippedItemInfo2.iconScale) : UInt), mAssetLoadingComponent);
	...
	function showNotAllowedPopup() {
		var _loc1_ = new DBUIOneButtonPopup(mDBFacade, Locale.getString("STACK_NOT_ALLOWED_TITLE"), Locale.getString("STACK_NOT_ALLOWED_MESSAGE"),
			Locale.getString("CANCEL"), mNotAllowedCallback);
		MemoryTracker.track(_loc1_, "DBUIOneButtonPopup - created in EquipItemToSlotPopup.showNotAllowedPopup()");
	}

	function equipToAvatarWeaponSlot(slotIndex:UInt) {
		mDBFacade.dbAccountInfo.inventoryInfo.equipItemOnAvatar((mAvatarInstanceId : UInt), mItemInfoToEquip.databaseId, slotIndex, mClosePopupCallback);
	}

	function equipToAvatarConsumableSlot(slotIndex:UInt) {
		...
		if (mDBFacade.dbAccountInfo.inventoryInfo.canEquipThisConsumable(_loc2_, slotIndex, _loc3_.gmId)) {
			mDBFacade.dbAccountInfo.inventoryInfo.equipConsumableOnAvatar((mAvatarInstanceId : UInt), _loc3_.gmStackable.Id, slotIndex, false,
				mClosePopupCallback);
		} else {
			showNotAllowedPopup();
		}

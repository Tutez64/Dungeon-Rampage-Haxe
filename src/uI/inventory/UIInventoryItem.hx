...
class UIInventoryItem extends UIButton {
	...
	public function new(dbFacade:DBFacade, parentSlot:MovieClip, templateClass:Dynamic, tooltipClass:Dynamic, clickedCallback:ASFunction) {
	...
	public function dragSwapItem(item:InventoryBaseInfo) {
		reset();
		mParentSlot.addChild(mRoot);
		info = item;
	}
	...
	override public function set_enabled(value:Bool):Bool {
		super.enabled = value;
		equippable = (value ? (1 : UInt) : (3 : UInt) : UInt);
		return value;
	}

	@:isVar public var equippable(get, set):UInt;

	public function set_equippable(value:UInt):UInt {
		mEquippable = value;

...
class DungeonRewardPanel {
	...
	public function new(dbFacade:DBFacade, rewardPanelRoot:MovieClip, dungeonSummary:DistributedDungeonSummary, selectedCallback:ASFunction) {
		mDBFacade = dbFacade;
		mRewardPanel = rewardPanelRoot;
	...
	function populateRewardPanel() {
		...
		if (_loc5_ > 0) {
			...
			if (mDungeonSummary.isSingleChestList[0]) {
				_loc2_ = new DungeonRewardElement(mDBFacade, ASCompat.dynamicAs((mRewardPanel : ASAny).equip_slot_3.graphic, flash.display.MovieClip), _loc3_,
					mSelectedCallback);
			} else {
				_loc2_ = new DungeonRewardElement(mDBFacade, ASCompat.dynamicAs((mRewardPanel : ASAny).equip_slot_1.graphic, flash.display.MovieClip), _loc3_,
					mSelectedCallback);
			}
		...
		if (_loc4_ > 0) {
			...
			_loc3_.setFromDungeonSummary();
			_loc2_ = new DungeonRewardElement(mDBFacade, ASCompat.dynamicAs((mRewardPanel : ASAny).equip_slot_2.graphic, flash.display.MovieClip), _loc3_,
				mSelectedCallback);
			mDungeonRewardElements.push(_loc2_);
		}
	}

	public function setChestAsSelected(chestGM:GMChest) {
		var _loc2_ = 0;
		_loc2_ = 0;
		while (_loc2_ < mDungeonRewardElements.length) {
			if (mDungeonRewardElements[_loc2_].chestInfo.gmChestInfo == chestGM) {
	...
	public function removeChestFromInventory(chestInfo:ChestInfo) {
		var _loc2_ = 0;
		_loc2_ = 0;
		while (_loc2_ < mDungeonRewardElements.length) {
			if (mDungeonRewardElements[_loc2_].chestInfo.gmId == chestInfo.gmId) {
	...
	public function highlightChest(chestInfo:ChestInfo) {
		var _loc2_ = 0;
		_loc2_ = 0;
		while (_loc2_ < mDungeonRewardElements.length) {
			if (mDungeonRewardElements[_loc2_].chestInfo.gmId == chestInfo.gmId) {

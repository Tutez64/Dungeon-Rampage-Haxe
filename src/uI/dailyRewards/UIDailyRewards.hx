...
class UIDailyRewards extends DBUIPopup {
	...
	public function new(dbFacade:DBFacade, closeCallback:ASFunction, forceOpen:Bool) {
		...
		mAssetLoadingComponent.getSoundAsset(DBFacade.buildFullDownloadPath("Resources/Audio/soundEffects.swf"), "ExpCollectSound",
			function(param1:SoundAsset) {
				mSoundGoldPlunk = param1;
			});
		mAssetLoadingComponent.getSoundAsset(DBFacade.buildFullDownloadPath("Resources/Audio/soundEffects.swf"), "TEMP_DooberGoldLarge1",
			function(param1:SoundAsset) {
				mSoundGoldStart = param1;
			});
		mAssetLoadingComponent.getSoundAsset(DBFacade.buildFullDownloadPath("Resources/Audio/soundEffects.swf"), "DailyRewardShred",
			function(param1:SoundAsset) {
				mSoundBox = param1;
			});
	...
	function goldSlideTimerTick(event:TimerEvent) {
	...
	function startGoldCount() {
		...
		mGoldCountTotal = ASCompat.toInt(ASCompat.toNumber(ASCompat.dynGetIndex(mRewardInfo[2],
			ASCompat.toNumber(mRewardInfo[0]) - 1)) * ASCompat.toNumber(mRewardInfo[1]));
	...
	function goldCountTimerTick(event:TimerEvent) {
		...
		if (mCountGold >= mGoldCountTotal) {
			...
			ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).crewBonus_anim.crewBonus.header_crew_bonus_number, "text",
				ASCompat.toNumber(mRewardInfo[1]) - 1);
			mGoldCountTimer.removeEventListener("timer", goldCountTimerTick);
			mGoldCountTimer.stop();
			startMysterySlide();
		}
	...
	function darkenGoldText(event:TimerEvent) {
		if (mDBFacade == null) {
			return;
		}
	...
	function mysterySlideTimerTick(event:TimerEvent) {
	...
	function mysteryFadeTimerTick(event:TimerEvent) {
		var _loc2_ = 0;
		if (mBoxAlpha > 0) {
	...
	function dailyRewardPopupLoaded(swfAsset:SwfAsset) {
		...
		mCrewBonusController = new MovieClipRenderController(mDBFacade,
			ASCompat.dynamicAs((mDailyRewardGoldPopup : ASAny).crewBonus_anim, flash.display.MovieClip));
		...
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).crewBonus_anim.crewBonus.header_crew_bonus_number, "text", "");
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).crewBonus_anim.crewBonus.tooltip.title_label, "text",
			Locale.getString("TEAM_BONUS_TOOLTIP_TITLE"));
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).crewBonus_anim.crewBonus.tooltip.description_label, "text",
			Locale.getString("TEAM_BONUS_TOOLTIP_DESCRIPTION"));
	...
	function getNextDay():Date {
		...
		ASCompat.ASDate.setTime(_loc1_, _loc1_.getTime()
			+ 86400000
			- (_loc1_.getSeconds() * 1000 + _loc1_.getMinutes() * 60000 + _loc1_.getHours() * 3600000));
		return _loc1_;
	}

	function startTimer(secondsToGo:Int) {
		var now:Date;
		if (mCountdownTimer != null) {
			return;
		}
	...
	function timerTick(event:TimerEvent) {
		...
		ASCompat.setProperty((mDailyRewardMysteryBoxPopup : ASAny).label_timer, "text",
			Std.string(_loc7_.getHours())
			+ ":"
			+ zeroPad(Std.int(_loc7_.getMinutes()), 2)
			+ ":"
			+ zeroPad(Std.int(_loc7_.getSeconds()), 2));
	...
	public function resetMysteryBox(paytoPlay:Bool = false) {
		stopTimer();
		mAcceptButton.visible = false;
		mReplayButton.visible = false;
		mPaytoReplay = paytoPlay;
	...
	function coinsLoaded(swfAsset:SwfAsset) {
		...
		setRewardAmounts(ASCompat.toInt(mRewardInfo[0]), ASCompat.toInt(mRewardInfo[1]), ASCompat.dynamicAs(mRewardInfo[2], Array),
			ASCompat.toInt(mRewardInfo[3]), ASCompat.toInt(mRewardInfo[4]), smallCoinClass, mediumCoinClass, largeCoinClass);
	}

	function hilightButton(buttonToHilight:UIButton) {
		var _loc2_ = new GlowFilter((16763972 : UInt), 1, 7, 7, 3, 1, false, false);
		_loc2_.quality = 2;
		buttonToHilight.root.filters = cast([_loc2_]);
	}

	function unhilightButton(buttonToHilight:UIButton) {
		var _loc2_ = new GlowFilter((0 : UInt), 0, 5, 5, 1, 1, false, false);
		_loc2_.quality = 2;
		buttonToHilight.root.filters = null;
	}

	function setRewardAmounts(concurrentDays:Int, numHeros:Int, baseValue:Array<ASAny>, secondsUntilNextReward:Int, costPlay:Int, smallCoinClass:Dynamic,
			mediumCoinClass:Dynamic, largeCoinClass:Dynamic) {
		...
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).label_number01, "text", baseValue[0]);
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).label_number02, "text", baseValue[1]);
		ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).label_number03, "text", baseValue[2]);
		mCostPlay = costPlay;
		ASCompat.setProperty((mDailyRewardMysteryBoxPopup : ASAny).button_replay.l_numberabel, "text", costPlay);
		if (secondsUntilNextReward > 0 && 1 != 0) {
			mSkipGoldCount = true;
			ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).label_number, "text", ASCompat.toNumber(baseValue[concurrentDays - 1]) * numHeros);
			ASCompat.setProperty((mDailyRewardGoldPopup : ASAny).crewBonus_anim.crewBonus.header_crew_bonus_number, "text", Std.string((numHeros - 1)));
		} else {
		...
		var _loc14_ = ASCompat.dynamicAs(ASCompat.createInstance(smallCoinClass, []), flash.display.MovieClip);
		var _loc15_ = ASCompat.dynamicAs(ASCompat.createInstance(mediumCoinClass, []), flash.display.MovieClip);
		var _loc16_ = ASCompat.dynamicAs(ASCompat.createInstance(largeCoinClass, []), flash.display.MovieClip);
		...
		if (secondsUntilNextReward > 0) {
			_loc9_ = [0, 0, 0];
			displayRewards(0, _loc9_, secondsUntilNextReward, false);
		}
	}

	function requestDailyRewards(boxPicked:Int) {
	...
	function displayRewards(boxPicked:Int, rewards:Array<ASAny>, timeToNext:Int, gotRefund:Bool) {
		var offer0:GMOffer;
		var offer1:GMOffer;
		var offer2:GMOffer;
		if (mDBFacade == null) {
	...
	function finalRewardReveal(rewardIndex:Int, timeToNext:Int) {
		var _loc4_:TextFormat = null;
		var _loc3_ = 0;
		_loc3_ = 0;
		while (_loc3_ < 3) {
			if (rewardIndex != _loc3_) {
		...
		ASCompat.setProperty(mRewardArray[rewardIndex], "visible", true);
		mSlotMovie.visible = false;
		startBoxFade();
		if (mNoRoomInStorageForReward) {
			ASCompat.setProperty((mDailyRewardMysteryBoxPopup : ASAny).label_message2, "text",
				Locale.getString("DAILY_REWARDS_YOU_GOT_REFUND") + mRewardName.toUpperCase() + Locale.getString("DAILY_REWARDS_STORAGE_FULL"));
			_loc4_ = new TextFormat();
			_loc4_.size = 14;
			(mDailyRewardMysteryBoxPopup : ASAny).label_message2.setTextFormat(_loc4_);
		} else {
			ASCompat.setProperty((mDailyRewardMysteryBoxPopup : ASAny).label_message2, "text",
				Locale.getString("DAILY_REWARDS_YOU_GOT") + mRewardName.toUpperCase());
		}

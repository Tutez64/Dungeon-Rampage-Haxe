...
class UIShopWeaponOffer extends UIShopOffer {
	...
	public function new(dbFacade:DBFacade, templateClass:Dynamic, descTooltipClass:Dynamic, chargeTooltipClass:Dynamic, buyCallback:ASFunction = null) {
		...
		if (mPower != null) {
			...
			ASCompat.setProperty((mPower : ASAny).attack_label, "text", Locale.getString("POWER"));
		}
		...
		mWeaponDescTooltip = new UIWeaponDescTooltip(mDBFacade, descTooltipClass);
	...
	public function setRolloverCallbacks(over:ASFunction, out:ASFunction) {
		mRollOverCallback = over;
		mRollOutCallback = out;
	}
	...
	override public function showOffer(gmOffer:GMOffer, gmHero:GMHero) {
		...
		weaponName = "";
		weaponName = GameMasterLocale.getGameMasterSubString("WEAPON_AESTHETIC_NAME", gmOffer.getOfferItemConstant(mDBFacade.gameMaster));
			...
			if (ASCompat.stringAsBool(swfPath) && ASCompat.stringAsBool(iconName)) {
				mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath(swfPath), function(param1:brain.assetRepository.SwfAsset) {
					...
					if (_loc3_ != null) {
						...
						mTapTooltip.setValues(GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_TAP_TITLE", mGMWeaponItem.Constant),
							GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_TAP_DESCRIPTION", mGMWeaponItem.Constant));
			...
			if (ASCompat.stringAsBool(swfPath1) && ASCompat.stringAsBool(iconName1)) {
				mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath(swfPath1), function(param1:brain.assetRepository.SwfAsset) {
					...
					if (_loc3_ != null) {
						...
						_loc4_ = mGMWeaponItem.WeaponController != null ? GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_CONTROLLER",
							mGMWeaponItem.WeaponController) : mGMWeaponItem.HoldTitle;
						mHoldTooltip.setValues(_loc4_, GameMasterLocale.getGameMasterSubString("WEAPON_ITEM_HOLD_DESCRIPTION", mGMWeaponItem.Constant));
		...
		if (mGMOfferDetail.Modifier1 != null) {
			mUIModifierList.push(new UIModifier(mDBFacade, ASCompat.dynamicAs((mRoot : ASAny).modifier_icon_1, flash.display.MovieClip),
				mGMOfferDetail.Modifier1));
		}
		if (mGMOfferDetail.Modifier2 != null) {
			mUIModifierList.push(new UIModifier(mDBFacade, ASCompat.dynamicAs((mRoot : ASAny).modifier_icon_2, flash.display.MovieClip),
				mGMOfferDetail.Modifier2));
		}
		if (mGMOfferDetail.Modifier3 != null) {
			mUIModifierList.push(new UIModifier(mDBFacade, ASCompat.dynamicAs((mRoot : ASAny).modifier_icon_3, flash.display.MovieClip),
				mGMOfferDetail.Modifier3, (0 : UInt), true));
		}
		preNameModifiers = "";
		i = 0;
		while (i < mUIModifierList.length) {
			...
			preNameModifiers += GameMasterLocale.getGameMasterSubString("MODIFIER_NAME", mUIModifierList[i].gmModifier.Constant).toUpperCase() + " ";
			i = i + 1;
		}
		weaponName = preNameModifiers
			+ GameMasterLocale.getGameMasterSubString("WEAPON_AESTHETIC_NAME", gmOffer.getOfferItemConstant(mDBFacade.gameMaster)).toUpperCase();
		if (mGMOfferDetail.Rarity == "LEGENDARY") {
		...
		if (mGMOfferDetail.Rarity == "LEGENDARY") {
			weaponName = GameMasterLocale.getGameMasterSubString("WEAPON_AESTHETIC_NAME", gmOffer.getOfferItemConstant(mDBFacade.gameMaster, "LEGENDARY", true))
				.toUpperCase();
		}
	...
	override function get_offerDescription():String {
		return mGMWeaponItem != null ? GameMasterLocale.getGameMasterSubString("WEAPON_AESTHETIC_DESCRIPTION",
			mGMWeaponItem.getWeaponAesthetic(mGMOfferDetail.Level).WeaponItemConstant) : "";
	}

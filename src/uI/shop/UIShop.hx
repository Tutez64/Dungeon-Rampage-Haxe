...
class UIShop {
	...
	public function new(dbFacade:DBFacade, swfAsset:SwfAsset, rootClip:MovieClip, townHeader:TownHeader) {
		mDBFacade = dbFacade;
		mSwfAsset = swfAsset;
		mTownHeader = townHeader;
		mSceneGraphComponent = new SceneGraphComponent(mDBFacade, "UIShop");
		mAssetLoadingComponent = new AssetLoadingComponent(mDBFacade);
		mLogicalWorkComponent = new LogicalWorkComponent(mDBFacade, "UIShop");
		mEventComponent = new EventComponent(mDBFacade);
		mWeaponsLogicalWorkComponent = new LogicalWorkComponent(mDBFacade, "UIShop");
	...
	public function refresh(startAtCategoryTab:String = "") {
		var owned:Bool;
		var popup:DBUIPopup;
		...
		mSelectedHero = ASCompat.dynamicAs(mDBFacade.gameMaster.heroById.itemFor(mDBFacade.dbAccountInfo.activeAvatarInfo.avatarType),
	...
	function specialSortComparator(gmOfferA:GMOffer, gmOfferB:GMOffer):Int {
		var _loc4_ = gmOfferA.isOnSaleNow != null;
		var _loc3_ = gmOfferB.isOnSaleNow != null;
		if (_loc4_ && _loc3_) {
			return (gmOfferA.Id - gmOfferB.Id:Int);
		}
		if (_loc4_ && !_loc3_) {
			return -1;
		}
		...
		if (gmOfferA.isNew && !gmOfferB.isNew) {
			return -1;
		}
		if (gmOfferB.isNew && !gmOfferA.isNew) {
			return 1;
		}
		return (gmOfferA.Id - gmOfferB.Id:Int);
	}

	function potionSortComparator(gmOfferA:GMOffer, gmOfferB:GMOffer):Int {
		return Std.int(gmOfferA.Price - gmOfferB.Price);
	}

	function keySortComparator(gmOfferA:GMOffer, gmOfferB:GMOffer):Int {
		var _loc6_ = gmOfferA.Details[0];
		var _loc5_ = gmOfferB.Details[0];
		...
		var _loc4_ = gmOfferA.isAvailableTime();
		var _loc3_ = gmOfferB.isAvailableTime();
		if (_loc4_ && !_loc3_) {
			return -1;
		}
		if (_loc3_ && !_loc4_) {
	...
	function weaponSortComparator(gmOfferA:GMOffer, gmOfferB:GMOffer):Int {
		if (gmOfferA.IsBundle && gmOfferB.IsBundle) {
			return Std.int(gmOfferA.Price - gmOfferB.Price);
		}
		if (gmOfferA.IsBundle && !gmOfferB.IsBundle) {
			return -1;
		}
		if (gmOfferB.IsBundle && !gmOfferA.IsBundle) {
			return 1;
		}
		var _loc4_ = gmOfferA.Details[0];
		var _loc3_ = gmOfferB.Details[0];
		...
		if (gmOfferA.Details[0].Level == gmOfferB.Details[0].Level) {
			return _loc5_ - _loc8_;
		}
		return (gmOfferA.Details[0].Level - gmOfferB.Details[0].Level:Int);
	}
	...
	function setupUI(rootClip:MovieClip) {
		...
		mRoot = rootClip;
		mPagination = new UIPagingPanel(mDBFacade, (0 : UInt), ASCompat.dynamicAs((mRoot : ASAny).pagination, flash.display.MovieClip),
			mSwfAsset.getClass("pagination_button"), this.setCurrentPage);
		...
		mDLCOfferTemplateClass = mSwfAsset.getClass("shop_offer_dlc_template");
		...
		mHeroWithEquipPicker = new HeroWithEquipPicker(mDBFacade, ASCompat.dynamicAs((mRoot : ASAny).hero_picker, flash.display.MovieClip),
			weaponTooltipClass, heroTooltipClass, heroSelected);
	...
	function displayCategoryPage() {
		...
		while (i < startIndex + numOffersPerPage) {
			...
			if (gmOffer.CurrencyType == "PREMIUM" && ASCompat.floatAsBool(gmOffer.Price)) {
				templateClass = mPremiumOfferTemplateClass;
			} else if (gmOffer.CurrencyType == "DLC") {
				templateClass = mDLCOfferTemplateClass;
			} else {
				templateClass = mOfferTemplateClass;
			}
	...
	function createTodaysShopTimerUI(gmOffer:GMOffer) {
		...
		if (gmOffer != null) {
			...
			mWeaponTabEndDate = gmOffer.EndDate;
	...
	function updateRemainingTimeText(clock:GameClock = null) {
	...
	function chestRevealResponse(obj:ASObject) {
	...
	function onWeaponRollOver(gmWeaponItem:GMWeaponItem, power:UInt) {
		mHeroWithEquipPicker.showWeaponComparison(gmWeaponItem, power);
	}

	function onWeaponRollOut(gmWeaponItem:GMWeaponItem, power:UInt) {
		mHeroWithEquipPicker.hideWeaponComparison();
	}

	public function showTab(category:String, page:UInt = (0 : UInt)) {
		...
		if (mCurrentTab == "WEAPON") {
			page = this.firstWeaponPageForActiveHero();
		}
	...
	function firstWeaponPageForActiveHero():UInt {
		...
		var _loc7_ = (0 : UInt);
		var _loc4_ = ASCompat.dynamicAs(mSelectedHero != null ? mDBFacade.dbAccountInfo.inventoryInfo.getAvatarInfoForHeroType(mSelectedHero.Id) : null,
			account.AvatarInfo);
		if (_loc4_ != null) {
	...
	function refreshPagination(page:UInt) {
		...
		mPagination.currentPage = mCurrentPage = (Std.int(_loc2_ != 0 ? (Std.int(Math.min(_loc2_ - 1, page)) : UInt) : (0 : UInt)) : UInt);
	...
	public function jumpToOffer(offerId:UInt):Bool {
		var _loc6_ = ASCompat.dynamicAs(mDBFacade.gameMaster.offerById.itemFor(offerId), gameMasterDictionary.GMOffer);
		if (_loc6_ == null) {
			return false;
		}
	...
	function categoryHasAnyNewOffers(category:String):Bool {
		var _loc2_:GMOffer = null;
		var _loc3_ = (mFilteredGMOffers.itemFor(category) : Vector<GMOffer>);
		if (_loc3_ == null) {
							...
							if (!ASCompat.toBool(_loc2_.IsCoinAltOffer)) {
								if (!(_loc10_ == "SPECIAL" && StoreServicesController.alreadyOwns(mDBFacade, _loc2_))) {
									if (!(ASCompat.toBool(_loc2_.IsBundle) && StoreServicesController.alreadyOwns(mDBFacade, _loc2_))) {
										...
										if (!(_loc6_ != null
											&& _loc6_.Hidden
											&& !mDBFacade.dbConfigManager.getConfigBoolean("want_hidden_heroes", false))) {

...
class JSONTokenizer {
	...
	final function readString():JSONToken {
		...
		while (true) {
			...
			if (_loc1_ >= 0) {
				_loc3_ = 0;
				_loc4_ = ASCompat.toInt(_loc1_ - 1);
		...
		var _loc2_ = JSONToken.create(JSONTokenType.STRING, this.unescapeString(this.jsonString.substr(this.loc, ASCompat.toInt(_loc1_ - this.loc))));
		this.loc = ASCompat.toInt(_loc1_ + 1);
		this.nextChar();
		return _loc2_;
	}
	...
	public function unescapeString(input:String):String {
		...
		var _loc3_ = 0;
		_loc4_ = 0;
		var _loc5_ = input.length;
		do {
			_loc3_ = input.indexOf("\\", _loc4_);
			if (_loc3_ < 0) {
				_loc2_ += input.substr(_loc4_);
				break;
			}
			_loc2_ += input.substr(_loc4_, _loc3_ - _loc4_);
			_loc4_ = _loc3_ + 2;
			_loc6_ = input.charAt(_loc3_ + 1);
			switch (_loc6_) {
				case "\"":
					_loc2_ += _loc6_;
	...
	final function isWhiteSpace(ch:String):Bool {
		if (ch == " " || ch == "\t" || ch == "\n" || ch == "\r") {
			return true;
		}
	...
	final function isDigit(ch:String):Bool {
		return ch >= "0" && ch <= "9";
	}

	final function isHexDigit(ch:String):Bool {
		return this.isDigit(ch) || ch >= "A" && ch <= "F" || ch >= "a" && ch <= "f";
	}

	final public function parseError(message:String) {

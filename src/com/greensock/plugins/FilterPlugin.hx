...
class FilterPlugin extends TweenPlugin {
	...
	function initFilter(props:ASObject, defaultFilter:BitmapFilter, propNames:Array<ASAny>) {
		...
		_loc6_ = propNames.length;
		while (ASCompat.toBool(_loc6_--)) {
			_loc5_ = propNames[_loc6_];
			if (props.hasOwnProperty(_loc5_) && (_filter : ASAny)[_loc5_] != props[_loc5_]) {
				if (_loc5_ == "color" || _loc5_ == "highlightColor" || _loc5_ == "shadowColor") {
					_loc7_ = new HexColorsPlugin();
					_loc7_.initColor(_filter, _loc5_, (ASCompat.toInt((_filter : ASAny)[_loc5_]) : UInt), (ASCompat.toInt(props[_loc5_]) : UInt));
					...
					(_filter : ASAny)[_loc5_] = props[_loc5_];
				} else {
					addTween(_filter, _loc5_, ASCompat.toNumber((_filter : ASAny)[_loc5_]), props[_loc5_], _loc5_);
				}
			}
		}
	}

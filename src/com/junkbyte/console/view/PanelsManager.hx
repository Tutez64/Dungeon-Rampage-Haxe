...
class PanelsManager {
	...
	public function addPanel(panel:ConsolePanel) {
	...
	public function removePanel(n:String) {
		var _loc2_ = ASCompat.reinterpretAs(this.console.getChildByName(n), ConsolePanel);
		if (_loc2_ != null) {
			_loc2_.close();
		}
	}
	...
	public function getPanel(n:String):ConsolePanel {
		return ASCompat.reinterpretAs(this.console.getChildByName(n), ConsolePanel);
	}
	...
	public function panelExists(n:String):Bool {
		return ASCompat.reinterpretAs(this.console.getChildByName(n), ConsolePanel) != null ? true : false;
	}

	public function setPanelArea(panelname:String, rect:Rectangle) {
		var _loc3_ = this.getPanel(panelname);
		if (_loc3_ != null) {
			_loc3_.x = rect.x;
			_loc3_.y = rect.y;
			if (ASCompat.floatAsBool(rect.width)) {
				_loc3_.width = rect.width;
			}
			if (ASCompat.floatAsBool(rect.height)) {
				_loc3_.height = rect.height;
			}
		}
	}
	...
	public function update(paused:Bool, lineAdded:Bool) {
	...
	public function updateGraphs(graphs:Array<ASAny>) {
	...
	public function removeGraph(group:GraphGroup) {
		var _loc2_:GraphingPanel = null;
		if (this._fpsPanel != null && group == this._fpsPanel.group) {
			this._fpsPanel.close();
			this._fpsPanel = null;
		} else if (this._memPanel != null && group == this._memPanel.group) {
			this._memPanel.close();
			this._memPanel = null;
		} else {
			_loc2_ = ASCompat.dynamicAs(this._graphsMap[group.name], com.junkbyte.console.view.GraphingPanel);
			if (_loc2_ != null) {
				_loc2_.close();
				ASCompat.deleteProperty(this._graphsMap, group.name);
			}
		}
	}
	...
	function set_displayRoller(n:Bool):Bool {
	...
	function set_channelsPanel(b:Bool):Bool {
	...
	public function tooltip(str:String = null, panel:ConsolePanel = null) {
		...
		var _loc7_ = Math.NaN;
		var _loc8_ = Math.NaN;
		if (ASCompat.stringAsBool(str) && !this.rulerActive) {
			_loc3_ = (cast str.split("::"));
			str = _loc3_[0];
			if (_loc3_.length > 1) {
				str += "<br/><low>" + Std.string(_loc3_[1]) + "</low>";
			}
			...
			if (panel != null) {
				...
				_loc5_ = new Rectangle(panel.x, panel.y, panel.width, panel.height);
	...
	public function get_rulerActive():Bool {
		return this._ruler != null && this.console.contains(this._ruler) ? true : false;
	}

	function onRulerExit(e:Event) {
		if (this._ruler != null && this.console.contains(this._ruler)) {
	...
	function onPanelStartDragScale(e:Event) {
		var _loc3_:Array<ASAny> = null;
		var _loc4_:Array<ASAny> = null;
		var _loc5_ = 0;
		var _loc6_ = 0;
		var _loc7_:ConsolePanel = null;
		var _loc2_ = ASCompat.dynamicAs(e.currentTarget, ConsolePanel);

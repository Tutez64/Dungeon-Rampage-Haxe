...
class ConsolePanel extends Sprite {
	...
	static function onTextFieldMouseOut(e:MouseEvent) {
		cast(e.currentTarget, TextField).dispatchEvent(new TextEvent(TEXT_ROLL));
	}

	static function onTextFieldMouseMove(e:MouseEvent) {
		var url:String;
		var index = 0;
	...
	function init(w:Float, h:Float, resizable:Bool = false, col:Float = -1, a:Float = -1, rounding:Int = -1) {
		this.bg.graphics.clear();
		this.bg.graphics.beginFill((ASCompat.toInt(col >= 0 ? (Std.int(col) : UInt) : this.style.backgroundColor) : UInt),
			a >= 0 ? a : this.style.backgroundAlpha);
		if (rounding < 0) {
	...
	override public function set_visible(b:Bool) {
		super.visible = b;
		dispatchEvent(new Event(VISIBLITY_CHANGED));
		return b;
	}

	override public function set_width(n:Float) {
		if (n < this.minWidth) {
			n = this.minWidth;
		}
		if (this.scaler != null) {
		...
		return this.bg.width = n;
	}

	override public function set_height(n:Float) {
		if (n < this.minHeight) {
			n = this.minHeight;
		}
		if (this.scaler != null) {
	...
	public function registerSnaps(X:Array<ASAny>, Y:Array<ASAny>) {
		this._snaps = [X, Y];
	}

	function registerDragger(mc:DisplayObject, dereg:Bool = false) {
		if (dereg) {
	...
	function onDraggerMouseDown(e:MouseEvent) {
	...
	function onDraggerMouseMove(e:MouseEvent = null) {
	...
	function onDraggerMouseUp(e:MouseEvent) {
		this.stopDragging();
	}
	...
	function stopDragging() {
		...
		if (this._resizeTxt != null && this._resizeTxt.parent != null) {
	...
	function set_scalable(b:Bool):Bool {
		var _loc2_ = (0 : UInt);
		if (b && this.scaler == null) {
	...
	function onScalerMouseDown(e:Event) {
	...
	function onScalerMouseUp(e:Event) {
	...
	function returnSnappedFor(X:Float, Y:Float):Point {
		return new Point(this.getSnapOf(X, true), this.getSnapOf(Y, false));
	}

	function getSnapOf(v:Float, isX:Bool):Float {
		var _loc6_ = Math.NaN;
		var _loc3_ = v + this.width;
		var _loc4_:Array<ASAny> = ASCompat.dynamicAs(this._snaps[isX ? 0 : 1], Array);
		var _loc5_ = this.style.panelSnapping;
		if (checkNullIteratee(_loc4_))
			for (_tmp_ in _loc4_) {
				_loc6_ = ASCompat.toNumber(_tmp_);
				if (Math.abs(_loc6_ - v) < _loc5_) {
					return _loc6_;
				}
	...
	function registerTFRoller(field:TextField, overhandle:ASFunction, linkHandler:ASFunction = null) {

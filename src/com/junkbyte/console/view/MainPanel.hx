...
class MainPanel extends ConsolePanel {
	...
	public function addMenu(key:String, f:ASFunction, args:Array<ASAny>, rollover:String) {
		if (ASCompat.stringAsBool(key)) {
			key = new compat.RegExp("[^\\w]*", "g").replace(key, "");
			if (f == null) {
				ASCompat.deleteProperty(this._extraMenus, key);
			} else {
				this._extraMenus[key] = ([f, args, rollover] : Array<ASAny>);
			}
	...
	function onMouseWheel(e:MouseEvent) {
		var _loc2_ = 0;
		if (this._shift) {
			_loc2_ = console.config.style.traceFontSize + (e.delta > 0 ? 1 : -1);
	...
	function onCmdPrefRollOverOut(e:MouseEvent) {
		console.panels.tooltip(e.type == MouseEvent.MOUSE_MOVE ? "Current scope::(CommandLine)" : "", this);
	}

	function onCmdPrefMouseDown(e:MouseEvent) {
		try {
	...
	public function requestLogin(on:Bool = true) {
	...
	public function update(changed:Bool) {
	...
	function _updateTraces(onlyBottom:Bool = false) {
		if (this._atBottom) {
			this.updateBottom();
		} else if (!onlyBottom) {
			this.updateFull();
	...
	function lineShouldShow(line:Log):Bool {
		return (this._priority == 0 || (line.priority : UInt) >= this._priority)
			&& (this.chShouldShow(line.ch)
				|| ASCompat.stringAsBool(this._filterText)
				&& this._viewingChannels.indexOf(Console.FILTER_CHANNEL) >= 0
				&& line.text.toLowerCase().indexOf(this._filterText) >= 0
				|| this._filterRegExp != null
				&& this._viewingChannels.indexOf(Console.FILTER_CHANNEL) >= 0
				&& this._filterRegExp.search(line.text) >= 0);
	}

	function chShouldShow(ch:String):Bool {
		return (this._viewingChannels.length == 0 || this._viewingChannels.indexOf(ch) >= 0)
			&& (this._ignoredChannels.length == 0 || this._ignoredChannels.indexOf(ch) < 0);
	}
	...
	function setFilterText(str:String = "") {
	...
	function setFilterRegExp(expstr:String = "") {
		if (ASCompat.stringAsBool(expstr)) {
			this._filterText = null;
			this._filterRegExp = new compat.RegExp(LogReferences.EscHTML(expstr), "gi");
			this.startFilter();
		} else {
			this.endFilter();
		}
	...
	function makeLine(line:Log, showch:Bool):String {
		var _loc3_ = "<p>";
		if (showch) {
			_loc3_ += line.chStr;
		}
		...
		var _loc4_ = "p" + line.priority;
		return _loc3_ + "<" + _loc4_ + ">" + this.addFilterText(line.text) + "</" + _loc4_ + "></p>";
	}

	function addFilterText(txt:String):String {
		...
		if (this._filterRegExp != null) {
			...
			_loc2_ = this._filterRegExp.exec(txt);
			while (_loc2_ != null) {
				_loc3_ = ASCompat.toInt(_loc2_.index);
				_loc4_ = _loc2_[Std.string(0)];
				if (_loc4_.indexOf("<|>") >= 0) {
					this._filterRegExp.lastIndex -= _loc4_.length - _loc4_.indexOf("<|>");
				} else if (txt.lastIndexOf("<", _loc3_) <= txt.lastIndexOf(">", _loc3_)) {
					txt = txt.substring(0, _loc3_)
						+ "<u>"
						+ txt.substring(_loc3_, _loc3_ + _loc4_.length)
						+ "</u>"
						+ txt.substring(_loc3_ + _loc4_.length);
					this._filterRegExp.lastIndex += 7;
				}
				_loc2_ = this._filterRegExp.exec(txt);
			}
		} else if (ASCompat.stringAsBool(this._filterText)) {
			_loc5_ = txt.toLowerCase();
			_loc6_ = _loc5_.lastIndexOf(this._filterText);
			while (_loc6_ >= 0) {
				txt = txt.substring(0, _loc6_) + "<u>" + txt.substring(_loc6_, _loc6_ + this._filterText.length) + "</u>"
					+ txt.substring(_loc6_ + this._filterText.length);
	...
	function onTraceScroll(e:Event = null) {
		var _loc3_ = 0;
	...
	function onScrollbarDown(e:MouseEvent) {
		if (this._scroller.visible
			&& this._scroller.mouseY > 0
			|| !this._scroller.visible
			&& this._scroll.mouseY > this._scrollHeight / 2) {
			this._scrolldir = 3;
		} else {
			this._scrolldir = -3;
		}
	...
	function onScrollBarFrame(e:Event) {
		++this._scrolldelay;
		if (this._scrolldelay > 10) {
			this._scrolldelay = (9 : UInt);
			if (this._scrolldir < 0
				&& this._scroller.y > this._scroll.mouseY
				|| this._scrolldir > 0
				&& this._scroller.y + this._scroller.height < this._scroll.mouseY) {
				this._traceField.scrollV += this._scrolldir;
			}
		}
	}

	function onScrollBarUp(e:Event) {
	...
	function onScrollerUp(e:MouseEvent) {
	...
	override public function set_height(n:Float):Float {
		...
		if (height != n) {
			this._mini = n < (this._cmdField.visible ? _loc3_ + _loc2_ + 4 : _loc3_);
		}
		super.height = n;
		var _loc4_ = this._mini || !style.topMenu;
		this.updateTraceFHeight();
		this._traceField.y = _loc4_ ? 0 : _loc2_;
		this._traceField.height = n - (this._cmdField.visible ? _loc2_ + 4 : 0) - (_loc4_ ? 0 : _loc2_);
		var _loc5_ = n - (_loc2_ + 6);
		...
		this._bottomLine.y = this._cmdField.visible ? _loc5_ : n;
		this._scroll.y = _loc4_ ? 6 : _loc2_ + 4;
		var _loc6_ = style.controlSize;
		this._scrollHeight = ASCompat.toNumber(ASCompat.toNumber(this._bottomLine.y - ASCompat.toNumber(this._cmdField.visible ? 0 : _loc6_ * 2))
			- this._scroll.y);
	...
	public function updateMenu(instant:Bool = false) {
		if (instant) {
			this._updateMenu();
		} else {
			this._needUpdateMenu = true;
	...
	public function getChannelsLink(limited:Bool = false):String {
		...
		if (limited && _loc4_ > style.maxChannelsInMenu) {
		...
		while (_loc6_ < _loc4_) {
			...
			_loc8_ = !_loc5_
				...
				|| _loc5_
				&& _loc6_ != 0
				&& this.chShouldShow(_loc7_) ? "<ch><b>" + _loc7_ + "</b></ch>" : _loc7_;
	...
	function doActive(str:String, b:Bool):String {
		if (b) {
	...
	public function onMenuRollOver(e:TextEvent, src:ConsolePanel = null) {
		var _loc4_:Array<ASAny> = null;
		var _loc5_:ASObject = null;
		if (src == null) {
			src = this;
		}
		var _loc3_ = ASCompat.stringAsBool(e.text) ? StringTools.replace(e.text, "event:", "") : "";
		...
		console.panels.tooltip(_loc3_, src);
	}
	...
	public function onChannelPressed(chn:String) {
		var _loc2_:Array<ASAny> = null;
		if (this._ctrl && chn != Console.GLOBAL_CHANNEL) {
			_loc2_ = this.toggleCHList(this._ignoredChannels, chn);
			Reflect.callMethod(this, this.setIgnoredChannels, _loc2_);
		} else if (this._shift && chn != Console.GLOBAL_CHANNEL && this._viewingChannels[0] != LogReferences.INSPECTING_CHANNEL) {
			_loc2_ = this.toggleCHList(this._viewingChannels, chn);
	...
	function toggleCHList(current:Array<ASAny>, chn:String):Array<ASAny> {
		current = current.copy();
		var _loc3_ = current.indexOf(chn);
		if (_loc3_ >= 0) {
			current.splice(_loc3_, (1 : UInt));
			if (current.length == 0) {
	...
	function incPriority(down:Bool) {
		var _loc3_ = (0 : UInt);
		var _loc2_ = (10 : UInt);
		var _loc4_ = console.logs.last;
	...
	function commandKeyDown(e:KeyboardEvent) {
	...
	function commandKeyUp(e:KeyboardEvent) {
		...
		if (e.keyCode == Keyboard.ENTER) {
			...
			this.setHints();
			if (ASCompat.stringAsBool(this._cmdField.text) && this._cmdsInd < 0) {
	...
	function updateCmdHint(e:Event = null) {
	...
	function onCmdFocusOut(e:Event) {
		this.setHints();
	}

	function setHints(hints:Array<ASAny> = null) {
		var _loc2_:Array<ASAny> = null;
		...
		if (hints != null && hints.length != 0) {
			this._hint = ASCompat.dynGetIndex(hints[0], 0);
			if (hints.length > 1) {
				_loc5_ = ASCompat.dynGetIndex(hints[1], 0);
				_loc6_ = false;
				_loc7_ = 0;
	...
	public function updateCLScope(str:String) {
		if (this._enteringLogin) {

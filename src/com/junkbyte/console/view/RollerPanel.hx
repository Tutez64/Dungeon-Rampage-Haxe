...
class RollerPanel extends ConsolePanel {
	...
	function removeListeners(e:Event = null) {
	...
	function onMenuRollOver(e:TextEvent) {
		...
		var _loc2_ = ASCompat.stringAsBool(e.text) ? StringTools.replace(e.text, "event:", "") : "";
		if (_loc2_ == "close") {
			_loc2_ = "Close";
	...
	function keyDownHandler(e:KeyboardEvent) {
		...
		var _loc2_ = String.fromCharCode((e.charCode : Int));
		this.cancelCaptureKeySet();
		console.setRollerCaptureKey(_loc2_, e.shiftKey, e.ctrlKey, e.altKey);
		console.panels.tooltip(null);
	}
}

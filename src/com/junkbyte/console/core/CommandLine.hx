...
class CommandLine extends ConsoleCore {
	...
	public function new(m:Console) {
		...
		this._saved.set("C", m);
		remoter.registerCallback("cmd", function(bytes:ByteArray) {
			run(bytes.readUTF());
		});
		remoter.registerCallback("scope", function(bytes:ByteArray) {
	...
	public function set_base(obj:ASObject):ASObject {
		if (ASCompat.toBool(this.base)) {
			report("Set new commandLine base from " + Std.string(this.base) + " to " + Std.string(obj), 10);
		} else {
	...
	public function handleScopeString(bytes:ByteArray) {
		this._scopeStr = bytes.readUTF();
	}

	public function handleScopeEvent(id:UInt) {
		var _loc2_:ByteArray = null;
		var _loc3_:ASAny = /*undefined*/ null;
		if (remoter.remoting == Remoting.RECIEVER) {
			_loc2_ = new ByteArray();
			_loc2_.writeUnsignedInt(id);
			remoter.send("scope", _loc2_);
		} else {
			_loc3_ = console.refs.getRefById(id);
	...
	public function store(n:String, obj:ASObject, strong:Bool = false) {
		...
		n = new compat.RegExp("[^\\w]*", "g").replace(n, "");
	...
	public function getHintsFor(str:String, max:UInt):Array<ASAny> {
		...
		hints = ASCompat.ASArray.sort(hints, function(a:Array<ASAny>, b:Array<ASAny>):Int {
	...
	public function addCLCmd(n:String, callback:ASFunction, desc:String, allow:Bool = false, endOfArgsMarker:String = ";") {
		var _loc6_:Array<ASAny> = (cast n.split("|"));
		var _loc7_ = 0;
		while (_loc7_ < _loc6_.length) {
			n = _loc6_[_loc7_];
			this._slashCmds[n] = new SlashCommand(n, callback, desc, false, allow, endOfArgsMarker);
			if (_loc7_ > 0) {
				this._slashCmds.setPropertyIsEnumerable(n, false);
			}
			_loc7_ = ASCompat.toInt(_loc7_) + 1;
		}
	}

	public function addSlashCommand(n:String, callback:ASFunction, desc:String = "", alwaysAvailable:Bool = true, endOfArgsMarker:String = ";") {
		var _loc6_:SlashCommand = null;
		n = new compat.RegExp("[^\\w]*", "g").replace(n, "");
		if (this._slashCmds[n] != null) {
			_loc6_ = ASCompat.dynamicAs(this._slashCmds[n], SlashCommand);
			if (!_loc6_.user) {
				throw new Error("Can not alter build-in slash command [" + n + "]");
			}
		}
		if (callback == null) {
			ASCompat.deleteProperty(this._slashCmds, n);
		} else {
			this._slashCmds[n] = new SlashCommand(n, callback, LogReferences.EscHTML(desc), true, alwaysAvailable, endOfArgsMarker);
		}
	}
	...
	function onExecLineComplete(e:Event) {
		var _loc2_ = ASCompat.dynamicAs(e.currentTarget, Executer);
	...
	function execCommand(str:String) {
		var param:String;
	...
	public function setReturned(returned:ASAny, changeScope:Bool = false, say:Bool = true) {
		if (!config.commandLineAllowed) {
			report(DISABLED, 9);
			return;
		}
		if (returned != /*undefined*/ null) {
			this._saved.set(Executer.RETURNED, returned, true);
			if (changeScope && returned != this._scope) {
				this._prevScope.reference = this._scope;
	...
	public function sendCmdScope2Remote(e:Event = null) {
	...
	function reportError(e:Error) {
		...
		var _loc2_ = console.refs.makeString(e);
	...
	function saveCmd(param:String = null) {
		this.store(param, this._scope, false);
	}

	function saveStrongCmd(param:String = null) {
	...
	function stringCmd(param:String) {
	...
	function explodeCmd(param:String = "0") {
		var _loc2_ = ASCompat.toInt(param);
		console.explodech(console.panels.mainPanel.reportChannel, this._scope, _loc2_ <= 0 ? 3 : _loc2_);
	}

	function mapCmd(param:String = "0") {
		console.mapch(console.panels.mainPanel.reportChannel, ASCompat.dynamicAs(this._scope, DisplayObjectContainer), (ASCompat.toInt(param) : UInt));
	}

	function funCmd(param:String = "") {
		var _loc2_ = new FakeFunction(this.run, param);
...
private class FakeFunction {
	...
	public function new(r:ASFunction, l:String) {
		this.run = r;
		this.line = l;
	}
...
private class SlashCommand {
	...
	public function new(nn:String, ff:ASFunction, dd:String, cus:Bool, permit:Bool, argsMarker:String) {
		this.n = nn;
		this.f = ff;
		this.d = ASCompat.stringAsBool(dd) ? dd : "";
		this.user = cus;
		this.allow = permit;
		this.endMarker = argsMarker;
	}
}

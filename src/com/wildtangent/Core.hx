...
class Core extends Sprite {
	...
	public function checkTop() {
		var _loc1_ = root;
		var _loc2_ = parent;
		cast(_loc1_, flash.display.DisplayObjectContainer).setChildIndex(cast(_loc1_, flash.display.DisplayObjectContainer).getChildByName(_loc2_.name),
			ASCompat.toInt(ASCompat.toNumberField(_loc1_, "numChildren") - 1));
	}
}

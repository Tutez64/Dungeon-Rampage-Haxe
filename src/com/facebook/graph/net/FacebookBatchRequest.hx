...
class FacebookBatchRequest extends AbstractFacebookRequest {
	...
	public function new(batch:Batch, completeCallback:ASFunction = null) {
		super();
		this._batch = batch;
		_callback = completeCallback;
	}

	public function call(accessToken:String) {
		var __tmpAssignObj11:ASObject;
		...
		while (_loc7_ < _loc6_) {
			...
			if (ASCompat.toBool(_loc8_.params)) {
				...
				if (_loc8_.requestMethod == URLRequestMethod.GET || _loc8_.requestMethod.toUpperCase() == "DELETE") {
					__tmpAssignObj11 = _loc10_;
					ASCompat.setProperty(__tmpAssignObj11, "relative_url", Std.string(__tmpAssignObj11.relative_url) + "?" + _loc11_);
				} else {
		...
		if (!_loc4_) {
			...
			ASCompat.setProperty(_loc12_, "access_token", accessToken);
	...
	function sendPostRequest(requests:Array<ASAny>, files:Array<ASAny>) {
		...
		_loc3_.writePostData("batch", com.adobe.serialization.json.JSON.encode(requests));
		var _loc4_ = (requests.length : UInt);
		var _loc5_ = (0 : UInt);
		while (_loc5_ < _loc4_) {
			_loc6_ = requests[(_loc5_ : Int)];
			_loc7_ = files[(_loc5_ : Int)];

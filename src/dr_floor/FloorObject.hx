...
class FloorObject extends GameObject {
	...
	function set_distributedDungeonFloor(value:DistributedDungeonFloor):DistributedDungeonFloor {
		mDistributedDungeonFloor = value;
		this.updateTile();
		return value;
	}
	...
	public function set_tile(value:Tile):Tile {
		if (value == mTile) {
			return value;
		}
		if (mTile != null) {
	...
	function set_position(pos:Vector3D):Vector3D {
		mPosition = pos;
		this.updateTile();
		return pos;
	}
	...
	public function set_layer(layer:Int):Int {
		mLayer = layer;
	...
	public function set_view(floorView:FloorView):FloorView {
		return mFloorView = floorView;
	}
	...
	function createNavCollisions(constant:String) {
		var _loc2_ = mDistributedDungeonFloor.dungeonFloorFactory.tileFactory.propFactory.getNavCollisionJson(constant);
		this.processJsonNavCollisions(_loc2_, this.addNavCollision);
	}

	function processJsonNavCollisions(navCollisions:Array<ASAny>, addFunc:ASFunction) {
		...
		mFilter = buildFilter();
		if (checkNullIteratee(navCollisions))
			for (_tmp_ in navCollisions) {
				_loc7_ = _tmp_;
				_loc6_.identity();
				...
				_loc5_ = NavCollider.buildNavColliderFromJson(mDBFacade, _loc7_, this, _loc3_, _loc9_.z, _loc8_, this.mDistributedDungeonFloor.box2DWorld,
					mFilter);
				if (_loc5_ != null) {

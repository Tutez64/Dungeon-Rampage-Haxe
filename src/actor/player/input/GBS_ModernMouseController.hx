...
class GBS_ModernMouseController extends MouseController {
	...
	override function handleMouseDown(mouseDownEvent:MouseEvent) {
		super.handleMouseDown(mouseDownEvent);
	}
	...
	override function determineSelection() {
		...
		if (mMouseDownThisFrame) {
			...
			if (mMouseDownActorThisFrame != null && isValidTarget(mMouseDownActorThisFrame)) {
				...
				mHeroGameObjectOwner.distributedDungeonFloor.effectManager.playEffect(DBFacade.buildFullDownloadPath("Resources/Art2D/FX/db_fx_library.swf"),
					"UI_pointer_click", mClickedLocation);
			}
		}
	...
	function handleAlternateWeaponAttack(weaponIndex:Int) {
		clearMovement();
		faceLocation(getMouseWorldLocation());
		beginAttack(weaponIndex);
	}
	...
	function setCurrentTargetSelection(isSelected:Bool) {
		if (mCurrentTarget.actorView == null) {
			return;
		}
		if (isSelected) {
	...
	function isPrimaryWeaponRanged():Bool {
		...
		return mHeroGameObjectOwner.weapons[0].weaponData.ClassType == "SHOOTING"
			|| mHeroGameObjectOwner.weapons[0].weaponData.ClassType == "MAGIC";
	}

	function isValidTarget(actor:ActorGameObject):Bool {
		return !actor.isDead();
	}
	...
	function attackMoveToTarget(inTarget:ActorGameObject) {
		setCurrentTargetSelection(true);
		if (isHeroInWeaponRange(inTarget)) {
			beginAttack();
			zeroVelocity();
			if (!mDBFacade.inputManager.mouseDown) {
				clearCurrentTarget();
			...
			moveTowardsLocation(inTarget.actorView.position, true);
		}
	}

	function isHeroInWeaponRange(inTarget:ActorGameObject):Bool {
		if (isPrimaryWeaponRanged()) {
			return true;
		}
		var _loc3_:Float = 0.25;
		var _loc2_ = getHeroDistanceToObject(inTarget);
		return _loc3_ >= _loc2_;
	}

	public function getHeroDistanceToObject(gameObject:ActorGameObject):Float {
		if (mHeroGameObjectOwner.navCollisions.length == 0) {
			Logger.warn("getHeroDistanceToObject - Hero Game Object has no NavColliders");
			return 0;
		}
		if (gameObject.navCollisions.length == 0) {
			Logger.warn("getHeroDistanceToObject - GameObject has no NavColliders");
			return 0;
		}
		var _loc3_ = mHeroGameObjectOwner.navCollisions[0];
		var _loc2_ = gameObject.navCollisions[0];
	...
	public function beginAttack(weaponIndex:Int = 0) {
		mPotentialAttacksThisFrame.push(new Tuple(weaponIndex, false));
	}

	public function moveTowardsLocation(location:Vector3D, faceMovementDirection:Bool) {
		...
		var _loc4_ = location.subtract(_loc5_);
		_loc4_.normalize();
	...
	public function faceLocation(location:Vector3D) {
		var _loc2_ = location.subtract(mHeroGameObjectOwner.worldCenter);
		_loc2_.normalize();

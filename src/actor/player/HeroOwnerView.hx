...
class HeroOwnerView extends HeroView {
	...
	public function new(dbFacade:DBFacade, heroGameObject:HeroGameObject) {
		super(dbFacade, heroGameObject);
	}

	function fadeOccludingObjects(gameClock:GameClock) {
	...
	override public function init() {
		...
		if (!wantMouseEnabled) {
			...
			hitSprite.graphics.drawRect(-HIT_AREA_HALF_WIDTH, -HIT_AREA_HALF_HEIGHT + actor.ActorView.BODY_Y_OFFSET + HIT_AREA_Y_OFFSET,
				HIT_AREA_HALF_WIDTH * 2, HIT_AREA_HALF_HEIGHT * 2);
		...
		mAssetLoadingComponent.getSwfAsset(DBFacade.buildFullDownloadPath("Resources/Art2D/FX/db_fx_library.swf"),
		...
		mAssetLoadingComponent.getSoundAsset(DBFacade.buildFullDownloadPath("Resources/Audio/soundEffects.swf"), "TEMP_Heartbeat1",
			function(param1:SoundAsset) {
				mHeartbeatSoundEffect = param1;
			});
	}

	override public function setHp(hp:UInt, maxHp:UInt) {
		super.setHp(hp, maxHp);
		if (hp <= maxHp * ACTIVATE_HEARTBEAT_AT_HEALTH_LEVEL) {
	...
	override public function spawnDamageFloater(isCriticalHit:Bool, delta:Int, attackerIsOwner:Bool, attackeeIsOwner:Bool, effectiveness:Int,
			type:UInt = (0 : UInt), floaterType:String = "DAMAGE_MOVEMENT_TYPE") {
		var _loc8_:DamageFloater = null;
		if (mDBFacade.featureFlags.getFlagValue("want-damage-floaters")) {
			_loc8_ = new DamageFloater(mDBFacade, delta, mParentActorObject, (0 : UInt), (24 : UInt), 1.2, 90, null, attackerIsOwner, attackeeIsOwner, true,

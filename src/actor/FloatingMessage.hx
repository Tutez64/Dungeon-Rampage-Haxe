...
class FloatingMessage {
	...
	public function new(root:DisplayObject, dbFacade:DBFacade, holdDuration:UInt, totalDuration:UInt, scaleDampening:Float, floatSpeed:Float,
			floatDirection:Vector3D = null, finishedCallback:ASFunction = null, movementType:String = "DAMAGE_MOVEMENT_TYPE", resetPositionAtEnd:Bool = false) {
		mDBFacade = dbFacade;
		mRoot = root;
		...
		mScaleDampening = scaleDampening;
		mFloatSpeed = floatSpeed;
		mFloatDirection = floatDirection != null ? floatDirection : DEFAULT_DIRECTION;
		mResetPositionAtEnd = resetPositionAtEnd;
		if (mResetPositionAtEnd) {
			mStartPosition = new Vector3D(root.x, root.y);
		}
		if (movementType == "BUFF_DAMAGE_MOVEMENT_TYPE") {
			buffFloaterTween();
		} else {
			damageFloaterTween();
		}
	...
	function damageFloaterTween() {
		...
		mTimelineMax = new TimelineMax({
			"tweens": [
				TweenMax.to(mRoot, mTotalDuration * 0.8 * dt, {
	...
	function buffFloaterTween() {
		...
		mTimelineMax = new TimelineMax({
			"tweens": [
				TweenMax.to(mRoot, mTotalDuration * dt, {
					"x": toX,
					"scaleX": toScaleX,
					"scaleY": toScaleY
				}),

...
class HeroOwnerSensor implements IContactResolver {
	...
	public function new(dbFacade:DBFacade, distributedFloor:Floor, shape:B2Shape, team:UInt) {
		...
		_loc5_.groupIndex = -1;
		_loc5_.maskBits = DBGlobal.b2dMaskForTeam(team);
		...
		mBody = distributedFloor.box2DWorld.CreateBody(_loc6_);
		mBody.CreateFixture(_loc7_);
	}

	@:isVar public var position(never, set):Vector3D;

	public function set_position(pos:Vector3D):Vector3D {
		mBody.SetPosition(NavCollider.convertToB2Vec2(pos));
		return pos;
	}

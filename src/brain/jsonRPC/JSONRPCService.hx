...
class JSONRPCService {
	...
	public static function getFunction(name:String, url:String):ASFunction {
		return getMethod(name, url);
	}
}

function sanitizeForLog(str:String, methodName:String = null):String {
	if (ASCompat.stringAsBool(JSONRPCService.validationToken) && JSONRPCService.validationToken.length > 0) {
		str = str.split(JSONRPCService.validationToken).join("[Redacted]");
	}
	if (ASCompat.stringAsBool(methodName) && methodName.toLowerCase().indexOf("token") >= 0) {
		str = "[Redacted]";
	}
	return str;
}

function getMethod(name:String, url:String):ASFunction {
	var fn = Reflect.makeVarArgs(function(_rest:Array<ASAny>):ASAny {
		...
		req.method = "POST";
		if (ASCompat.stringAsBool(JSONRPCService.accountId) && ASCompat.stringAsBool(JSONRPCService.validationToken)) {
			req.requestHeaders = [];
			req.requestHeaders.push(new URLRequestHeader("X-Account-Id", JSONRPCService.accountId));
			req.requestHeaders.push(new URLRequestHeader("X-Validation-Token", JSONRPCService.validationToken));
			if (ASCompat.stringAsBool(JSONRPCService.initialLoginTraceId)) {
				req.requestHeaders.push(new URLRequestHeader("X-Session-Initial-Trace", JSONRPCService.initialLoginTraceId));
			}
		} else {
			Logger.warn("Attempting to make an RPC call to "
				+ url
				+ " but there are no authentication headers available yet. Proceeding without authentication headers, but the request may fail.");
		}
		...
		resp.addEventListener("ioError", function(param1:flash.events.IOErrorEvent) {
			Logger.error(Std.string(counter) + " IOErrorEvent: " + param1.toString() + " Data:" + sanitizeForLog(Std.string(req.data)) + " URL:"
				+ url.toString());
			inv.handleError(new Error(param1.text));
		});
		resp.addEventListener("securityError", function(param1:flash.events.SecurityErrorEvent) {
			Logger.error(Std.string(counter) + " SecurityErrorEvent: " + param1.toString() + " Data:" + sanitizeForLog(Std.string(req.data)) + " URL:"
				+ url.toString());
			inv.handleError(new Error(param1.text));
		});

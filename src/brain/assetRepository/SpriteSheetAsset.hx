...
class SpriteSheetAsset extends Asset {
	...
	public function FactoryCompiledIn(basename:String, dict:ASObject, cacheKey:String) {
		var _loc8_ = 0;
		var _loc9_ = Math.NaN;
		var _loc6_ = 0;
		var _loc4_ = 0;
		...
		mBaseName = basename;
		mCacheKeyFor = cacheKey;
		mCenters = new Vector<Point>();
		mFrames = new Vector<BitmapData>();
		mFrameWidth = (ASCompat.toInt(dict.width) : UInt);
		mFrameHeight = (ASCompat.toInt(dict.height) : UInt);
		mNumFramesY = (ASCompat.toInt(dict.offsets.length) : UInt);
		mNumFramesX = (ASCompat.toInt(dict.Timing.length) : UInt);
		mTimingVector = new Vector<Float>();
		_loc8_ = 0;
		while ((_loc8_ : UInt) < mNumFramesX) {
			_loc9_ = ASCompat.toNumber(ASCompat.toNumber(dict.Timing[_loc8_]) * 1000) / 24;
		...
		while ((_loc6_ : UInt) < mNumFramesY) {
			...
			while ((_loc4_ : UInt) < mNumFramesX) {
				_loc7_ = basename + "_" + Std.string(_loc6_) + "_" + Std.string(_loc4_) + ".png";
				...
				mCenters[_loc10_] = new Point(ASCompat.toNumber(mFrameWidth * 0.5
					- ASCompat.toNumberField(ASCompat.dynGetIndex(dict.offsets[_loc6_], _loc4_), "y")),
					ASCompat.toNumber(mFrameHeight * 0.5 - ASCompat.toNumberField(ASCompat.dynGetIndex(dict.offsets[_loc6_], _loc4_), "x")));
				_loc10_++;
				_loc4_++;
			}
			_loc6_++;
	...
	public function FactoryFromSWf(basename:String, dict:ASObject, swfAsset:SwfAsset) {
		var _loc8_ = 0;
		var _loc9_ = Math.NaN;
		var _loc6_ = 0;
		var _loc4_ = 0;
		...
		while ((_loc6_ : UInt) < mNumFramesY) {
			...
			while ((_loc4_ : UInt) < mNumFramesX) {
				_loc7_ = basename + "_" + Std.string(_loc6_) + "_" + Std.string(_loc4_) + ".png";
				_loc5_ = swfAsset.getClass(_loc7_, false);
				mFrames[_loc10_] = ASCompat.dynamicAs(ASCompat.createInstance(_loc5_, []), flash.display.BitmapData);
				mCenters[_loc10_] = new Point(ASCompat.toNumber(mFrameWidth * 0.5
					- ASCompat.toNumberField(ASCompat.dynGetIndex(dict.offsets[_loc6_], _loc4_), "y")),
					ASCompat.toNumber(mFrameHeight * 0.5 - ASCompat.toNumberField(ASCompat.dynGetIndex(dict.offsets[_loc6_], _loc4_), "x")));
	...
	function getFrameIndex(xIndex:UInt, yIndex:UInt):UInt {
		if (mNumFramesY == 1) {
			yIndex = (0 : UInt);
		}
		var _loc3_ = yIndex * mNumFramesX + xIndex;
		if (_loc3_ >= (mFrames.length : UInt)) {
			throw new Error("Tried to get frameIndex out of bounds: " + mBaseName + " " + _loc3_ + " / " + mFrames.length + " x:" + xIndex + " y:" + yIndex
				+ " t1:" + mNumFramesY + " t2:" + mNumFramesX);
		}
		return _loc3_;
	}

	public function getFrame(xIndex:UInt, yIndex:UInt):BitmapData {
		var _loc3_ = getFrameIndex(xIndex, yIndex);
		if ((mFrames.length : UInt) <= _loc3_) {
			Logger.error("Trying to access frame: "
				+ _loc3_
				+ " but there are only "
				+ mFrames.length
				+ "frames in animation named: "
				+ this.mBaseName);
			return null;
		}
	...
	public function getCenter(xIndex:UInt, yIndex:UInt):Point {
		var _loc3_ = getFrameIndex(xIndex, yIndex);

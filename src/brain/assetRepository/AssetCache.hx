...
final class AssetCache {
	...
	public function itemFor(key:AssetLoaderInfo):Asset {
		if (key.useCache) {
			return ASCompat.dynamicAs(mCachedAssets.itemFor(key.getKey()), brain.assetRepository.Asset);
		}
		return null;
	}

	public function add(key:AssetLoaderInfo, value:Asset):Bool {
		if (key.useCache) {
			return mCachedAssets.add(key.getKey(), value);
		}
		return false;
	}

	public function remove(asset:Asset):Bool {
		return mCachedAssets.remove(asset);
	}

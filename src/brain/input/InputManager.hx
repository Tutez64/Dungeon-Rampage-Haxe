...
class InputManager {
	...
	public function registerMenuNavigationCallback(callback:ASFunction) {
		menuNavigationCallback = callback;
	}

	public function unregisterKeyPressedCallback(callback:ASAny) {
		if (menuNavigationCallback != null) {
	...
	public function check(input:ASAny):Bool {
		return ASCompat.toNumber(input) < 0 ? mNumberOfKeysDown > 0 : mKeysDown[ASCompat.toInt(input)];
	}

	public function pressed(input:ASAny):Bool {
		return ASCompat.toBool(ASCompat.toNumber(input) < 0 ? mNumberOfKeysPressed != 0 : mKeysPressed.indexOf(ASCompat.toInt(input)) >= 0);
	}

	public function released(input:ASAny):Bool {
		return ASCompat.toBool(ASCompat.toNumber(input) < 0 ? mNumberOfKeysReleased != 0 : mKeysReleased.indexOf(ASCompat.toInt(input)) >= 0);
	}
	...
	function onKeyDown(e:KeyboardEvent = null) {
		var _loc2_ = 0;
		if (!disableInputs) {
			_loc2_ = lastKey = (e.keyCode : Int);
			if (!mKeysDown[_loc2_]) {
	...
	function onMouseWheel(e:CustomMouseWheelEvent) {
		if (!disableInputs) {
			mouseWheel = true;
			mMouseWheelDelta = e.delta;
		}
	}

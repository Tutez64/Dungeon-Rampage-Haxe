...
class TimeSpan {
	...
	public function new(milliseconds:Float) {
		_totalMilliseconds = Math.ffloor(milliseconds);
	}
	...
	public static function fromMilliseconds(milliseconds:Float):TimeSpan {
	...
	public static function fromMinutes(minutes:Float):TimeSpan {
		return new TimeSpan(minutes * 60000);
	}

	public static function fromHours(hours:Float):TimeSpan {
		return new TimeSpan(hours * 3600000);
	}

	public static function fromDays(days:Float):TimeSpan {
	...
	public function getTimeBetweenTimeSpanAndNow(countdownTimer:Bool = false):String {
		var _loc5_ = Date.now();
		...
		if (countdownTimer && _loc6_.totalMilliseconds < 0) {
			return "00:00:00";
		}
	...
	public function add(date:Date):Date {
		var _loc2_ = Date.fromTime(date.getTime());
